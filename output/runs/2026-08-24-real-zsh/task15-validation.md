...
`PASS` includes four representative invalid-input/auth cases whose expected result is a non-zero rejection. The 12 `FAIL` cases are retained real upstream or runtime-contract failures, not hidden retries.
...
- Upstream/runtime failures: latest novel is `malformed_upstream_response`; novel detail samples, artwork comments, bookmark detail, blocked users, and artwork series return `not_found`.

These failures do not invalidate the three targeted repairs, but they remain visible for Task 16 classification.
