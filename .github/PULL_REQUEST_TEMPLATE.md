...
日本語：この必須 metadata は Quality gate で検証されます。merge 後に bilingual release note を準備するためだけに使用し、feature PR で changelog/unreleased を編集しないでください。Added、Changed、Fixed、Security、Documentation、Maintenance、None から正確に 1 つ選びます。user-visible な compatibility break ごとに breaking を true にし、pre-1.0 release では minor version で表現できます。None には具体的な理由が必要です。
-->
