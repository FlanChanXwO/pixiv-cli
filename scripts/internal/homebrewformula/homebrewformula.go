// Package homebrewformula 根据已验证 release checksums.txt 的六个 archive
...
func readChecksums(path, version string) (map[string]string, error) {
	...
	expected := make(map[string]struct{}, len(targets)+2)
	for _, target := range targets {
		expected[releasecontract.ArchiveName(version, target)] = struct{}{}
	}
...
func renderFormula(template, formula, version string, checksums map[string]string) (string, error) {
	...
	for _, target := range releasecontract.FixedTargets() {
		...
		archive := releasecontract.ArchiveName(version, target)
