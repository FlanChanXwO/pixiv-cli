// Package prepublishhomebrew 检查 Homebrew 发布前验证与受保护恢复 workflow 的安全边界。
package prepublishhomebrew
...
func value(node *yaml.Node, key string) *yaml.Node {
	value, _ := workflowyaml.MappingValue(node, key)
	return value
}
...
func exactKeys(node *yaml.Node, keys ...string) bool {
	return workflowyaml.HasExactMappingKeys(node, keys...)
}
