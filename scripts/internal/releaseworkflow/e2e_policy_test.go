...
func TestReleaseWorkflowHasOfflineSDKContractGate(t *testing.T) {
	...
	if _, ok := workflowyaml.MappingValue(e2e, "environment"); ok {
		t.Fatal("offline e2e contract gate must not require a credential environment")
	}
	if workflowyaml.ContainsSecretReference(e2e) {
	...
	if workflowyaml.ContainsSecretReference(steps[0]) || workflowyaml.ContainsSecretReference(steps[1]) {
