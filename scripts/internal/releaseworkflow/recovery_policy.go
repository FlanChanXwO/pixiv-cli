...
func checkTagTrigger(root *yaml.Node) error {
	...
	dispatch, ok := workflowyaml.MappingValue(on, "workflow_dispatch")
	...
	inputs, ok := workflowyaml.MappingValue(dispatch, "inputs")
	if !ok || requireOnlyMappingKeys(inputs, "release_tag") != nil {
		return errors.New("workflow_dispatch must contain only the exact release_tag input")
	}
	releaseTag, ok := workflowyaml.MappingValue(inputs, "release_tag")
	if !ok || requireOnlyMappingKeys(releaseTag, "description", "required", "type") != nil || workflowyaml.RequireScalar(releaseTag, "required", "true") != nil || workflowyaml.RequireScalar(releaseTag, "type", "string") != nil {
...
func checkRecoveryPolicy(root *yaml.Node) error {
	...
	if !ok || requireOnlyMappingKeys(env, "RELEASE_TAG") != nil || workflowyaml.RequireScalar(env, "RELEASE_TAG", "${{ github.event_name == 'workflow_dispatch' && inputs.release_tag || github.ref_name }}") != nil {
	...
	buildEnv, ok := workflowyaml.MappingValue(build, "env")
	if !ok || requireOnlyMappingKeys(buildEnv, "CC", "RUSTUP_TOOLCHAIN", "GIT_CONFIG_COUNT", "GIT_CONFIG_KEY_0", "GIT_CONFIG_VALUE_0") != nil || workflowyaml.RequireScalar(buildEnv, "CC", "${{ matrix.cc }}") != nil || workflowyaml.RequireScalar(buildEnv, "RUSTUP_TOOLCHAIN", "${{ matrix.rust_toolchain }}") != nil || workflowyaml.RequireScalar(buildEnv, "GIT_CONFIG_COUNT", "1") != nil || workflowyaml.RequireScalar(buildEnv, "GIT_CONFIG_KEY_0", "core.autocrlf") != nil || workflowyaml.RequireScalar(buildEnv, "GIT_CONFIG_VALUE_0", "false") != nil {
...
func requireCanonicalBuildSteps(steps []*yaml.Node) error {
	...
	if err := workflowyaml.RequireScalar(steps[7], "name", "Apply the audited test-only recovery overlay"); err != nil {
...
func requireOverlayQualitySequence(steps []*yaml.Node, overlayIndex, freshCheckoutIndex int) error {
	...
	for offset, gate := range expected {
		...
		if err := workflowyaml.RequireScalar(step, "name", gate.name); err != nil {
