...
func TestCheckWorkflowRejectsSecurityAndQualityPolicyMutations(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			mutate: func(t *testing.T, root *yaml.Node) {
				t.Helper()
				removeRunFragment(t, stepWithRun(t, jobNode(t, root, "publish"), "go run ./scripts/releaseassets finalize"), "--release-sources internal/update/source/release_sources.txt")
			},
		},
		{
