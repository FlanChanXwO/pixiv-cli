...
func checkVerifyReleaseSourceJob(job *yaml.Node) error {
	...
	env, ok := workflowyaml.MappingValue(ancestryGate, "env")
	if !ok || requireOnlyMappingKeys(env, "DEFAULT_BRANCH") != nil || workflowyaml.RequireScalar(env, "DEFAULT_BRANCH", "${{ github.event.repository.default_branch }}") != nil {
...
func requireCanonicalCheckout(step *yaml.Node, jobName string, requirements ...checkoutWithRequirement) error {
	...
	if err := workflowyaml.RequireScalar(step, "uses", canonicalCheckoutAction); err != nil {
		return fmt.Errorf("%s must use the canonical checkout", jobName)
	}
	...
	for _, requirement := range requirements {
		if err := workflowyaml.RequireScalar(with, requirement.key, requirement.value); err != nil {
			return fmt.Errorf("%s must use the canonical checkout", jobName)
		}
	}
	return nil
...
func checkPublishJob(job *yaml.Node) (int, []*yaml.Node, error) {
	...
	if err := workflowyaml.RequireScalar(job, "environment", "release"); err != nil {
...
func checkSigningSecretReachability(validate, build, productionBuild, releaseNotesAudit, verifyReleaseSource, publish *yaml.Node, publishSteps []*yaml.Node, signingIndex int) error {
	...
	for index := 0; index+1 < len(publish.Content); index += 2 {
		if publish.Content[index].Value != "steps" && workflowyaml.ContainsSecretReference(publish.Content[index+1]) {
	...
	for index, step := range publishSteps {
		if index != signingIndex && workflowyaml.ContainsSecretReference(step) {
			return errors.New("publish job must not reference secrets outside its signing metadata step")
		}
...
func checkSigningStep(step *yaml.Node) error {
	env, ok := workflowyaml.MappingValue(step, "env")
...
func requireExpectedSigningSecret(env *yaml.Node, key string) error {
	value, ok := workflowyaml.MappingValue(env, key)
...
func containsSigningSecretReferenceOutsideEnvironment(step *yaml.Node) bool {
	if step == nil || step.Kind != yaml.MappingNode {
		return workflowyaml.ContainsSecretReference(step)
	}
	for index := 0; index+1 < len(step.Content); index += 2 {
		if step.Content[index].Value == "env" {
			continue
		}
		if workflowyaml.ContainsSecretReference(step.Content[index+1]) {
			return true
		}
	}
	return false
...
func signingStepIndex(steps []*yaml.Node) (int, *yaml.Node) {
	for index, step := range steps {
		...
		if _, exists := workflowyaml.MappingValue(env, "RELEASE_SIGNING_PRIVATE_KEY"); exists {
			return index, step
		}
	}
	return -1, nil
