...
func checkE2EJob(job *yaml.Node) error {
	...
	if err := workflowyaml.RequireScalar(job, "name", "SDK E2E contract gate"); err != nil {
		return fmt.Errorf("e2e job: %w", err)
	}
	...
	if err := workflowyaml.RequireScalar(steps[2], "name", "Run offline SDK E2E contract tests"); err != nil {
		return errors.New("offline SDK contract step must keep its canonical name")
	}
	if err := workflowyaml.RequireScalar(steps[2], "shell", "bash"); err != nil || !equalCommands(splitCommands(requireRunValue(steps[2])), splitCommands(offlineE2ECommands)) {
...
func requireExactStringSequence(mapping *yaml.Node, key string, values ...string) error {
	sequence, ok := workflowyaml.MappingValue(mapping, key)
