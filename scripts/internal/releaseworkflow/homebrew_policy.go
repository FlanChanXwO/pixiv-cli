...
printf '%s\n' "$formula_name" > staging-formula/formula-name`
	...
	if err := workflowyaml.RequireScalar(job, "needs", "publish"); err != nil {
		return fmt.Errorf("render_homebrew_formula job: %w", err)
	}
...
`
	...
	if err := workflowyaml.RequireScalar(job, "needs", "render_homebrew_formula"); err != nil {
		return fmt.Errorf("verify_homebrew_formula job: %w", err)
	}
	...
	matrix, _ := workflowyaml.MappingValue(strategy, "matrix")
...
printf '%s\n' "$RELEASE_TAG" > skillhub-release-tag/release-tag`
	...
	if err := workflowyaml.RequireScalar(job, "needs", "verify_homebrew_formula"); err != nil {
		return fmt.Errorf("deploy_homebrew_tap job: %w", err)
	}
	if err := workflowyaml.RequireScalar(job, "runs-on", "ubuntu-24.04"); err != nil {
		return fmt.Errorf("deploy_homebrew_tap job: %w", err)
...
func checkHomebrewSecretReachability(render, verify, deploy *yaml.Node) error {
	if workflowyaml.ContainsSecretReference(render) || workflowyaml.ContainsSecretReference(verify) {
	...
	for index := 0; index+1 < len(deploy.Content); index += 2 {
		if deploy.Content[index].Value != "steps" && workflowyaml.ContainsSecretReference(deploy.Content[index+1]) {
	...
	for index, step := range steps {
		if index != protectedPushIndex && workflowyaml.ContainsSecretReference(step) {
			return errors.New("deploy_homebrew_tap job must not reference secrets outside its final push step")
		}
	...
	env, ok := workflowyaml.MappingValue(steps[protectedPushIndex], "env")
	...
	value, ok := workflowyaml.MappingValue(env, "HOMEBREW_TAP_DEPLOY_KEY")
