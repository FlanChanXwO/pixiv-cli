...
func checkWorkflow(body []byte) error {
	...
	if workflowyaml.ContainsSecretReference(root) {
...
func checkNativeEvidenceJob(job *yaml.Node) error {
	...
	if !ok || requireOnlyMappingKeys(env, "RUSTUP_TOOLCHAIN") != nil || workflowyaml.RequireScalar(env, "RUSTUP_TOOLCHAIN", "${{ matrix.rust_toolchain }}") != nil {
	...
	if !ok || strategy.Kind != yaml.MappingNode || requireOnlyMappingKeys(strategy, "fail-fast", "matrix") != nil || workflowyaml.RequireScalar(strategy, "fail-fast", "false") != nil {
...
func requireCanonicalCheckout(step *yaml.Node) error {
	if requireOnlyMappingKeys(step, "uses", "with") != nil || workflowyaml.RequireScalar(step, "uses", canonicalCheckoutAction) != nil {
	...
	if !ok || requireOnlyMappingKeys(with, "persist-credentials") != nil || workflowyaml.RequireScalar(with, "persist-credentials", "false") != nil {
		return errors.New("native evidence job must use the canonical credential-free checkout")
	}
	return nil
}
...
func requireCanonicalSetupGo(step *yaml.Node) error {
	if requireOnlyMappingKeys(step, "uses", "with") != nil || workflowyaml.RequireScalar(step, "uses", canonicalSetupGoAction) != nil {
		return errors.New("native evidence job must use the canonical Go setup action")
	}
	with, ok := workflowyaml.MappingValue(step, "with")
	if !ok || requireOnlyMappingKeys(with, "go-version") != nil || workflowyaml.RequireScalar(with, "go-version", "1.26.3") != nil {
		return errors.New("native evidence job must use the canonical Go setup action")
	}
	return nil
}
...
func requireDirectRunStep(step *yaml.Node, name, command string) error {
	if requireOnlyMappingKeys(step, "name", "shell", "run") != nil || workflowyaml.RequireScalar(step, "name", name) != nil || workflowyaml.RequireScalar(step, "shell", "bash") != nil || workflowyaml.RequireScalar(step, "run", command) != nil {
...
go test ./internal/media/ugoira -run '^TestRustUgoiraEncoderNativeGIFAndAPNG$' -count=1
`
...
cp "internal/media/ugoira/rust/staticlib/${{ matrix.rust_target }}/$staticlib" "evidence/$staticlib"
...
func requireMultilineRunStep(step *yaml.Node, name, canonical string) error {
	if requireOnlyMappingKeys(step, "name", "shell", "run") != nil || workflowyaml.RequireScalar(step, "name", name) != nil || workflowyaml.RequireScalar(step, "shell", "bash") != nil {
...
func requireUploadEvidenceStep(step *yaml.Node) error {
	if requireOnlyMappingKeys(step, "name", "uses", "with") != nil || workflowyaml.RequireScalar(step, "name", "Upload native evidence") != nil || workflowyaml.RequireScalar(step, "uses", canonicalUploadArtifactAction) != nil {
		return errors.New("native evidence job must upload only its audited evidence artifact")
	}
	with, ok := workflowyaml.MappingValue(step, "with")
	if !ok || requireOnlyMappingKeys(with, "name", "path", "if-no-files-found") != nil || workflowyaml.RequireScalar(with, "name", "native-evidence-${{ matrix.artifact }}") != nil || workflowyaml.RequireScalar(with, "path", "evidence") != nil || workflowyaml.RequireScalar(with, "if-no-files-found", "error") != nil {
		return errors.New("native evidence job must upload only its audited evidence artifact")
	}
	return nil
}
...
func checkNativeEvidenceTrigger(root *yaml.Node) error {
	...
	push, hasPush := workflowyaml.MappingValue(on, "push")
	dispatch, hasDispatch := workflowyaml.MappingValue(on, "workflow_dispatch")
	...
	branches, hasBranches := workflowyaml.MappingValue(push, "branches")
	pathIgnores, hasPathIgnores := workflowyaml.MappingValue(push, "paths-ignore")
...
func checkContentsReadPermission(job *yaml.Node) error {
	...
	value, ok := workflowyaml.MappingValue(permissions, "contents")
