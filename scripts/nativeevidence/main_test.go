...
func TestConsolidateEvidencePublishesOnlyCompleteSameSourceStaticlibs(t *testing.T) {
	...
	if err := consolidateEvidence(consolidateOptions{inputDir: inputDir, outputDir: outputDir}); err != nil {
...
func TestConsolidateEvidenceRejectsMissingTargetWithoutPublishingOutput(t *testing.T) {
	...
	outputDir := filepath.Join(workDir, "staticlib")
	err := consolidateEvidence(consolidateOptions{inputDir: inputDir, outputDir: outputDir})
...
func writeConsolidationFixture(t *testing.T, inputDir, platform string, target nativeTarget, sourceDigest string) {
	...
	members := writeMinimalEvidenceArchive(t, archivePath, target.goos == "windows")
	...
	record := evidenceRecord{
		...
		Binary:       evidenceBinary{evidenceFile: evidenceFile{Name: binaryName, SHA256: binaryDigest}, Version: "v0.1.0-native-evidence.test", Commit: "fixture", BuildDate: "2026-07-12T00:00:00Z"},
