...
> 本文件覆盖 `G1-T01`、`G1-T02`、`G1-T03`、`G1-CHECK-01`、`G1-T04`、`G1-T05`、`G1-T06`、`G1-CHECK-02`、`G1-T07`、`G1-T08`、`G1-T09`、`G1-CHECK-03`、`G1-T10`、`G1-T11`、`G1-T12`、`G1-CHECK-04`、`G1-T13`、`G1-T14`、`G1-T15`、`G1-CHECK-05`、`G1-T16`、`G1-T17`、`G1-T18`、`G1-CHECK-06`、`G1-T19`、`G1-T20`、`G1-T21`、`G1-CHECK-07`、`G1-T22`、`G1-T23`、`G1-T24`、`G1-CHECK-08`、`G1-T25`、`G1-T26`、`G1-T27`、`G1-CHECK-09`、`G1-T28`、`G1-CORR-G1-T28-RECOMMENDED-01`、`G1-T29`、`G1-CORR-G1-T29-BOOKMARK-DETAIL-01`、`G1-CORR-G1-T29-NOVEL-COMMENTS-DATA-PROBE-01` 与后续 correction/integration gate，记录当前分支的代码、测试、历史和 Goal-3 证据，不授予任何 capability 的发布资格。
...
| 17 | `artwork-bookmark-mutation` | implemented_unverified | verified | verified | not_applicable | verified | verified | verified | verified | implemented_unverified | missing |
...
| 28 | `novel-comments-mutation` | implemented_unverified | verified | verified | verified | verified | verified | verified | verified | implemented_unverified | missing |
...
| 9 `novel-series` | yes | `GET /v2/novel/series`；旧 v1 必须 rejected | 有效 series ID；`last_order` 首页+续页；**required** | 已认证账号；series 至少两页 | N/A | recovery probe 使用显式候选完成 v2 首页 30 条 → SDK opaque cursor 第二页 22 条；旧 v1 rejection 与当前 path 保持；候选不固化为永久 fixture | `mapped_to_task`：G1-T20、G1-T23、G1-T28；recovery：`G1-RECOVER-G1-T28-SERIES-TARGET-01` |
...
| 26 `artwork-comments-mutation` | yes | `POST /v1/illust/comment/add`、`POST /v1/illust/comment/delete`；reply/stamp 共用 add | text/reply/stamp 各最小一条并 delete 本轮 comment；**no** | 明确授权隔离账号；目标 artwork 可评论；必须取得可靠 comment ID | 同账号 read-back；只删除本轮 comment ID；ID 不确定不得猜删/重放 | adapter/SDK/offline transport/MCP mutation 有；T30 comments slice 无显式 `CanComment` target，`writes=0`，无 read-back/cleanup 可执行；归类 `blocked_external(data/permission)` | `mapped_to_task`：G1-T14、G1-T17、G1-T18、G1-T30；correction：`CAND-G1-T06-REC-COMMENTS-MUTATION` |
| 27 `novel-comments-read` | yes | 当前生产 `GET /v2/novel/comments`；v3 为 candidate | 非空 comments；DTO/access-control；有 continuation 时完成第二页；**required if returned** | 已认证账号；目标 novel 可访问且有 comments；需可取得跨页数据，缺数据按 external blocker 记录 | N/A | `date` wire correction 后，候选 `29100695` 返回 2 条可映射 comments、无 continuation；scalar access-control 语义仍未确认，但 read mapping 已不再报 `invalid comment time` | `mapped_to_task`：G1-T29、G1-CORR-G1-T29-NOVEL-COMMENTS-DATA-PROBE-01、G1-CORR-G1-T29-COMMENT-WIRE-02 |
| 28 `novel-comments-mutation` | yes | `POST /v1/novel/comment/add`、`POST /v1/novel/comment/delete` | text/stamp/delete 本轮 comment；**no** | 明确授权隔离账号；目标 novel 可评论；保存响应 ID | 同账号 read-back；只删除本轮 ID；uncertain 不 replay | adapter/SDK/offline transport/MCP mutation 有；T30 comments slice 无显式 `CanComment` target，`writes=0`，无 read-back/cleanup 可执行；归类 `blocked_external(data/permission)` | `mapped_to_task`：G1-T15、G1-T17、G1-T18、G1-T30；correction：`CAND-G1-T06-REC-COMMENTS-MUTATION` |
| 29 `stamps` | yes | `GET /v1/stamps`；写入通过 artwork/novel comment add stamp | 至少一次 stamps read；#26/#28 各覆盖一条 stamp；**no** | read 需真实 stamp target；mutation 需隔离账号和可评论目标 | 同 comment mutation：read-back、只清理本轮 comment/stamp ID | stamps read 返回 40 项并完成正数 ID 选择；comment/stamp 写入受 #26/#28 target blocker 约束，未发写请求 | `mapped_to_task`：G1-T14、G1-T15、G1-T30；correction：`CAND-G1-T06-REC-COMMENTS-MUTATION` |
...
| 41 `recommended-all` | yes | artwork `/v1/illust/recommended`、novel `/v1/novel/recommended`、user `/v1/user/recommended`，CLI/MCP `kind=all` 四分区 | 四流 aggregate；每个可分页流实际 continuation；统一 budget、任一路失败整体失败；**required** | 已认证；四流均需非空或按 manifest 记录 data blocker；不得输出 partial success | N/A（read-only aggregate） | CLI/MCP offline aggregate 有；SDK `RecommendedAll` missing；#4/#11 单路 continuation P1 已闭合，但四流 strict aggregate/live contract 仍缺 | `mapped_to_task`：G1-T11、G1-T20、G1-T22、G1-T28、G1-T29；correction：`CAND-G1-T06-RECOMMENDED-ALL-SURFACE` |
...
- **External/decision blocker：** 当前 #27 `novel-comments-read` 存在真实 data/upstream blocker：当前 search page 的 comments 候选返回 `invalid comment time`，无法形成合法非空 SDK target；无 decision blocker。未来 live 数据/账号/权限/网络不足时，严格按 manifest 记录 `blocked_external`；不得把 internal bug 归类为 external blocker。
...
- **blocked_external (data/upstream)：** #27 novel comments——现已遍历当前 search page，但候选 comments DTO 返回 `invalid comment time`，没有可由 SDK 合法映射的非空 target；#20 bookmarked case——账号无 novel bookmarks。
...
- **Correction：** `G1-CORR-G1-T29-BOOKMARK-DETAIL-01` 与 `G1-CORR-G1-T29-NOVEL-COMMENTS-DATA-PROBE-01` 均已完成；前者收敛 absent bookmark detail，后者移除 `Items[:3]` 并安全遍历当前 page。**下一步：** G1-T30。
...
## 39. G1-CORR-G1-T29-NOVEL-COMMENTS-DATA-PROBE-01 novel comments 数据探测修正

- **结论：** PASS。target selection 不再依赖无依据的固定三项切片；当前 search page 的全部真实返回项可安全探测，首个非空 comments 目标命中后立即停止，无目标时返回 data-limited 状态。
...
## 41. G1-CHECK-10 Phase F exit：live validation + terminal routing

- **审计结论：** 当前不能将 Phase F 标记 `verified`，也不能把 required live acceptance 伪装成 accepted。G1-T28、G1-T29、G1-T30 均已到 terminal status；既有 correction `G1-CORR-G1-T28-RECOMMENDED-01`、`G1-CORR-G1-T29-BOOKMARK-DETAIL-01`、`G1-CORR-G1-T29-NOVEL-COMMENTS-DATA-PROBE-01` 均已 verified，没有新的可执行 correction task。除本 CHECK 外，任务图没有仍可执行的 `pending/in_progress` required work；G1-T31 与 G1-FINAL 被本 CHECK 的失败条件依赖锁定，G1-TERM 是唯一可执行的 terminal task。
- **Live verified：** G1-T28 的 feed/search/ranking/recommended/detail 场景（#1–4、#6–8、#10–13）有当前 live evidence；G1-T29 的 bookmark/user/relationship/MyPixiv/stamps/aggregate read 场景（#14–16、#18–19、#23–24、#29–37）按 manifest 完成；G1-T30 的 follow mutation 与 stamps read-back verified。partial aggregate、offline evidence 与数据受限场景均未被升级成完整 accepted。
- **External blockers：** #5 artwork-series、#9 novel-series 缺少可安全构造的真实 series target/第二页；#20 novel bookmarked=true 缺目标数据；#27 novel comments 候选返回 `invalid comment time`，无法形成合法非空 target；#17/#21 bookmark status-only write 后 detail/list/tags read-back 不可确认但 cleanup/reconcile 均干净；#26/#28 没有显式 `CanComment` 且可安全映射的 artwork/novel target，因此没有发出 comment write。上述均是当前账号、目标数据或上游条件，未归类为内部 bug。
- **Decision / scope blockers：** #6 `ugoira-metadata` CLI/MCP、#12 `novel-ranking` MCP、#23/#24/#41 public aggregate SDK/strict aggregate evidence，以及 #38 bare-ID、#39 rating MCP surface 仍只有 correction registry 或 frozen boundary，没有已批准的 public contract/owner；继续实现需要新增 surface、选择 breaking/范围方案或补充产品决策，不能在本 CHECK 自行扩大 scope。#41 不能因已有 CLI/MCP aggregate 或单路 recommended live 证据而 accepted。
...
## 43. Blocked closure superseded：comments wire 新证据恢复执行

- **GoalState：** 恢复 `ACTIVE`。历史 `BLOCKED_DECISION` checkpoint 保留审计价值，但不再是当前执行状态；原因是新的可复现 evidence 推翻了 G1-TERM 的关键前提 `runnable_required_tasks == 0`。
- **#27 reclassification：** `invalid comment time` 不再归类为纯 external。当前 endpoint DTO 从 `created_at` 取评论时间，而当前 App API 参考模型使用 `date`；public SDK 随后严格解析 normalized `CreateDate`，因此真实 comment 的 `date` 未被读取时会稳定形成当前 live 错误。已登记 `G1-CORR-G1-T29-COMMENT-WIRE-02`，状态 `pending`。
- **#26/#28 reclassification pending：** “没有满足 `CanComment` 的安全目标”暂时不能作为最终 external 结论。当前 adapter 期待 `access_control` object，而当前 App API 参考模型给出 `comment_access_control` integer；mutation harness 又只接受显式 `AccessControl.CanComment=true`。必须先证实 scalar 语义并纠正 wire mapping；禁止猜测整数含义或默认放行评论写入。纠正后若仍无安全 target，才重新记录 `blocked_external(data/permission)`。
...
## 47. G1-CHECK-10 Phase F exit audit（当前记录：2026-09-13）

- **Status / verdict：** `blocked_decision`（task terminal）；不是 Phase F `verified`，也不代表 Goal 完成。G1-T28、G1-T29、三项既有 correction、series recovery 与 G1-T30 均已到 terminal `verified`；当前没有新的内部 correction 或可执行 required task，下一张可执行控制任务为 `G1-TERM`。
- **Manifest / task graph：** 41 行 manifest ID `1..41` 各出现一次；`live_required=yes/no=36/5`；`mapped_to_task=41`、`unmapped=0`、`undecomposed=0`。G1-T31 依赖本 CHECK `verified`，因此当前不可执行；G1-FINAL 继续锁定。
- **当前 live 收敛：** #9 `novel-series` 已由显式 recovery candidate 完成首页 30 → SDK opaque cursor 第二页 22；#27 `date` wire correction 后成功映射当前候选 2 条 comments；#36 follow 已完成同账号 add/read-back/delete/read-back；#4/#11 recommended continuation P1 已闭合；#29 stamps read 成功。上述证据未扩展为未满足条件的 aggregate、mutation 或 public-surface acceptance。
- **External blockers：** #5 artwork-series 仍无可追溯目标；#17/#21 bookmark status-only 写入后的状态不可确认（cleanup/reconcile 干净，按 uncertain 不 replay）；#20 novel bookmark detail 缺 `bookmarked=true` 目标；#26/#28 comments mutation 在当前 wire 事实下仍无显式且可安全判定的 `CanComment` target，`writes=0`，没有评论/回复/stamp 写入。上述属于当前账号、目标数据或权限条件，未发现生产代码根因。
- **Decision / scope blockers：** #6、#12、#23、#24、#41、#38、#39 的 layer/public-surface 裁定尚未得到用户明确批准；继续执行会新增 CLI/MCP/SDK contract、改变 frozen boundary 或扩大 aggregate/rating/ID probe 范围，本 CHECK 不自行决定。`comment_access_control` 的 `0/1` scalar 语义同样未被猜测映射。
...
- **Git / push：** closure 前 Local/Remote=`955c09a90407d3baf0cda8b52fe252b575ec6594`，`origin/main=7ff1e6b4e6177c657876f69cd930d279d2d0bfce`，worktree clean、无远端分叉。G1-TERM closure checkpoint=`0ccf341b5dd1e26a953f1a57b5c5707807bf7a7c`，从 `955c09a90407d3baf0cda8b52fe252b575ec6594` 普通 fast-forward 推送成功；GitHub API 与 `git ls-remote` 均确认 Remote SHA == closure checkpoint。后续仅发布 documentation ledger。
- **Next：** 无可执行 required task；等待 external data/permission 或用户 scope 决策。不得执行 G1-T31/G1-FINAL，除非 blocker 条件解除并重新打开相应 task。

## 49. Resumed execution：comments/bookmark live evidence（2026-09-13）

- **GoalState：** `ACTIVE`。§48 的 `BLOCKED_DECISION` 是历史 closure；当前存在已验证的 external recovery evidence 与新的 artwork-series wire correction，故 `runnable_required_tasks == 0` 前提已被 supersede。
- **#17 artwork bookmark：** 非 Flan 账号 `127975236` 对新候选 `149605267` 完成 add → detail `restrict=public` 且 tags 读取成功 → public bookmark list 可见 → remove → detail/list/tags 均确认无残留。更早候选 `149605331` 的一次尝试遇到本地 SQLite lock，不计入证据且未 replay。
- **#20/#21 novel bookmark：** 同一账号对 `29100695` 完成 add → detail `restrict=public`、list 可见 → remove → detail/list/tags 均确认无残留；该 detail read-back 同时提供 `bookmarked=true` 等价 live 状态，解除 #20 target 与 #21 mutation read-back blocker。
- **#26 artwork comments：** 非 Flan 账号 `128042145`、artwork `149603743` 的 text/reply/stamp 均完成写入、专用 read-back、cleanup；结果均 `writes=1/read_back=true/cleanup=true/uncertain=false`。
- **#28 novel comments：** 同一账号、novel `29100695` 的 text/stamp 均完成写入、read-back、cleanup；结果均 `writes=1/read_back=true/cleanup=true/uncertain=false`。
- **安全边界：** 评论正文只使用极短赞美 `很棒！`；cleanup 依赖接口返回 ID；未把 token、cookie、refresh token、raw signed URL 或长评论正文写入日志/账本。默认账号已恢复为原先的 `127975236`。
- **状态变化：** #17/#20/#21/#26/#28 的历史 data/permission blocker 已以当前 live evidence 解除；#29 stamps 既有 evidence 保留。#5 artwork-series 不再只是“没有候选”问题，当前显式候选暴露出可复现的 `offset` continuation parser correction，登记为 `G1-CORR-G1-T28-ARTWORK-SERIES-WIRE-02`。
...
- **Ledger push：** 文档审计 checkpoint=`797bb91e76e3bfd6a357f746be8794d8d48c0afb`，从 `54e2b198fb0aa4ce290c30267b08c1c9edb030ba` 普通 fast-forward 推送成功；git ls-remote 核对 Remote SHA == Local HEAD。该 checkpoint 不是最终 closure push。
