...
- 修改 CLI/MCP tool、配置键、环境变量、输出语义、下载/认证/代理流程时，同步更新 `README.md`（英文，默认）与 `README.zh-CN.md` 或 `docs/`；涉及命令语义时同步检查 `skills/pixiv/`。
...
- 产品 skill（教 agent 使用 `pixiv` CLI，面向使用者分发）：`skills/pixiv/`，全英文；改 CLI 命令/flag/语义时同步更新。
