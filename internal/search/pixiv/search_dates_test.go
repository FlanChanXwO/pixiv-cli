...
func TestQuickDateRangeUsesTokyoDateAndClampsMonthEnd(t *testing.T) {
	...
	start, end, ok := searchpixiv.QuickDateRange("within_half_year", now)
...
func TestQuickDateRangeClampsLeapDayForYear(t *testing.T) {
	...
	start, end, ok := searchpixiv.QuickDateRange("within_year", now)
...
func TestQuickDateRangeRejectsUnknownValue(t *testing.T) {
	if _, _, ok := searchpixiv.QuickDateRange("within_last_month", time.Now()); ok {
