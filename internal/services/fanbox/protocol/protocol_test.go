...
func newTestSession(t *testing.T, handler http.Handler, cookie string) *protocol.Session {
	t.Helper()
	...
	session, err := protocol.NewSessionWithHTTPClient(cookie, &http.Client{Transport: transport})
	if err != nil {
		t.Fatalf("protocol.NewSessionWithHTTPClient() error = %v", err)
	}
	return session
}
...
func TestAPIRedirectDropsCookie(t *testing.T) {
	...
	if _, err := testPost(session, context.Background(), "post-1"); err != nil {
...
func TestStatusClassification(t *testing.T) {
	...
	cases := []struct {
		...
		{"/unauthorized", protocol.ErrNotAuthenticated},
		{"/forbidden", protocol.ErrForbidden},
		{"/challenge-body", protocol.ErrChallenge},
		{"/challenge-html", protocol.ErrChallenge},
		{"/challenge-header", protocol.ErrChallenge},
		{"/notfound", nil},
	}
	for _, test := range cases {
		t.Run(test.path, func(t *testing.T) {
			_, err := testHome(session, context.Background(), "https://api.fanbox.cc"+test.path)
			if test.want == nil {
				if err == nil {
					t.Fatal("endpoint unexpectedly succeeded")
				}
				if errors.Is(err, protocol.ErrForbidden) || errors.Is(err, protocol.ErrChallenge) || errors.Is(err, protocol.ErrNotAuthenticated) {
...
func TestNewSessionUsesBrowserTransportAndRejectsImplicitTransport(t *testing.T) {
	session, err := protocol.NewSession("FANBOXSESSID=tls-canary")
	if err != nil {
		t.Fatalf("protocol.NewSession() error = %v", err)
	}
	defer session.CloseIdleConnections()
	if _, err := protocol.NewSessionWithHTTPClient("FANBOXSESSID=tls-canary", &http.Client{}); err == nil {
		t.Fatal("protocol.NewSessionWithHTTPClient() accepted implicit standard transport")
	}
	if _, err := protocol.NewSession("FANBOXSESSID=tls-canary", protocol.WithHTTPClient(&http.Client{})); err == nil {
		t.Fatal("protocol.NewSession() with implicit client accepted implicit standard transport")
	}
}

func TestSessionUsesCustomUserAgent(t *testing.T) {
	session, err := protocol.NewSessionWithOptions("FANBOXSESSID=ua-canary", protocol.SessionOptions{
	...
	response, err := testHome(session, context.Background(), "https://api.fanbox.cc/post.info?postId=1")
...
func TestSessionRejectsUnsafeURLsBeforeTransport(t *testing.T) {
	called := false
	session, err := protocol.NewSessionWithHTTPClient("FANBOXSESSID=url-canary", &http.Client{Transport: roundTripFunc(func(*http.Request) (*http.Response, error) {
		called = true
		return nil, nil
	})})
	if err != nil {
