...
func (s *fakePoolState) SelectPixiv(_ context.Context, _ int64, attempted []int64, _ accountpixiv.Chooser) (accountpixiv.Account, error) {
	...
	if s.selectAt >= len(s.selections) {
		return accountpixiv.Account{}, poolpixiv.ErrAccountPoolExhausted
	}
...
func (s *fakePoolState) Freeze(_ context.Context, userID, until int64) error {
	s.frozen = append(s.frozen, userID)
	s.until = append(s.until, until)
	return nil
}

func scheduler(state *fakePoolState, now time.Time) poolpixiv.Scheduler {
	return poolpixiv.Scheduler{
...
func TestSchedulerFailsOverOnlyBeforeCommit(t *testing.T) {
	...
	err := scheduler(state, now).Run(context.Background(), func(_ context.Context, userID int64, _ *lifecycle.Attempt) error {
	...
	err = scheduler(state, now).Run(context.Background(), func(_ context.Context, _ int64, attempt *lifecycle.Attempt) error {
		attempt.Commit()
		return committed
	})
...
func TestSchedulerRequiresSafeFutureRetryAfter(t *testing.T) {
	...
	for _, retry := range []sdk.RetryAdvice{
		...
		err := scheduler(state, now).Run(context.Background(), func(context.Context, int64, *lifecycle.Attempt) error {
...
func TestSchedulerPreservesLastRateLimitOnExhaustion(t *testing.T) {
	...
	last := retryError(now)
	err := scheduler(state, now).Run(context.Background(), func(context.Context, int64, *lifecycle.Attempt) error { return last })
	require.ErrorIs(t, err, poolpixiv.ErrAccountPoolExhausted)
...
func TestChooseIsPureAndValidatesRandomIndex(t *testing.T) {
	...
	selected, err := poolpixiv.Choose(snapshot, config.AccountPoolStrategyRoundRobin, nil)
	require.NoError(t, err)
	require.Equal(t, int64(2), selected)
	selected, err = poolpixiv.Choose(snapshot, config.AccountPoolStrategyRandom, func(int) (int, error) { return 0, nil })
	require.NoError(t, err)
	require.Equal(t, int64(1), selected)
	_, err = poolpixiv.Choose(snapshot, config.AccountPoolStrategyRandom, func(int) (int, error) { return 9, nil })
	require.Error(t, err)
}
