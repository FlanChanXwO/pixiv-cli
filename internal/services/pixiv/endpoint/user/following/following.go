...
func (c *Client) List(ctx context.Context, request Request) (Result, error) {
	if c == nil || c.transport == nil {
		return Result{}, errors.New("user following transport is not configured")
	}
...
func (c *Client) list(ctx context.Context, query url.Values) (Result, error) {
	var raw responseDTO
	if err := c.transport.GetJSON(ctx, protocol.AppUserFollowing, query, &raw); err != nil {
		return Result{}, err
	}
...
func continuation(rawURL string) (int, error) {
	_, value, err := endpointcontinuation.Parse(rawURL, endpointcontinuation.Spec{
		Path:             protocol.AppUserFollowing,
		Keys:             []string{"offset"},
