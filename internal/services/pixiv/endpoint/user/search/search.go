...
func (c *Client) Search(ctx context.Context, request Request) (Result, error) {
	if c == nil || c.transport == nil {
		return Result{}, errors.New("user search transport is not configured")
	}
	...
	var raw responseDTO
	if err := c.transport.GetJSON(ctx, protocol.AppSearchUser, query, &raw); err != nil {
		return Result{}, err
	}
...
func continuation(rawURL string) (int, error) {
	_, value, err := endpointcontinuation.Parse(rawURL, endpointcontinuation.Spec{
		Path:             protocol.AppSearchUser,
		Keys:             []string{"offset"},
		AllowedQueryKeys: []string{"word"},
	})
