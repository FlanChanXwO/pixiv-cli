...
func (c *Client) List(ctx context.Context, request Request) (Result, error) {
	if c == nil || c.transport == nil {
		return Result{}, errors.New("user related transport is not configured")
	}
	query := url.Values{"seed_user_id": {strconv.FormatInt(request.SeedUserID, 10)}}
	if request.Offset > 0 {
		query.Set("offset", strconv.Itoa(request.Offset))
	}
	return c.list(ctx, protocol.AppUserRelated, query)
}
func (c *Client) list(ctx context.Context, path string, query url.Values) (Result, error) {
	var raw responseDTO
...
func continuation(rawURL string) (int, error) {
	_, value, err := endpointcontinuation.Parse(rawURL, endpointcontinuation.Spec{
		Path:             protocol.AppUserRelated,
		Keys:             []string{"offset"},
		AllowedQueryKeys: []string{"seed_user_id"},
	})
