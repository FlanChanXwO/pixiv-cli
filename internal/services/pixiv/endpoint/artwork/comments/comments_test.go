...
func TestCommentsListValidatesRequestAndRequiresCommentsList(t *testing.T) {
	tests := []struct {
		...
		{
			name:               "non-positive artwork ID",
			body:               `{"comments":[]}`,
			request:            comments.Request{ArtworkID: 0},
			wantMalformed:      false,
		...
		},
		{
			...
			body:               `{"comments":[]}`,
			request:            comments.Request{ArtworkID: 1, Offset: -1},
			wantMalformed:      false,
			wantNoTransportHit: true,
		},
		{
			...
			body:          `{}`,
			request:       comments.Request{ArtworkID: 1},
			wantMalformed: true,
		},
		{
			name:          "null comments",
...
func TestCommentsCreateBuildsFormAndMapsCommentID(t *testing.T) {
	...
	if transport.mutationForm.Get("illust_id") != "123" || transport.mutationForm.Get("comment") != "hello" || len(transport.mutationForm) != 2 {
...
func TestCommentsReplyBuildsFormAndMapsCommentID(t *testing.T) {
	...
	if transport.mutationForm.Get("illust_id") != "123" || transport.mutationForm.Get("comment") != "reply" || transport.mutationForm.Get("parent_comment_id") != "6" || len(transport.mutationForm) != 3 {
		t.Fatalf("mutation form = %#v", transport.mutationForm)
	}
}
...
func TestCommentsDeleteBuildsForm(t *testing.T) {
	...
	if transport.postFormCalls != 1 || transport.mutationPath != "/v1/illust/comment/delete" {
...
func TestCommentsMutationsRejectInvalidRequestsBeforeTransport(t *testing.T) {
	tests := []struct {
		...
		{
			name: "create non-positive artwork ID",
			run: func(transport *fakeTransport) error {
				_, err := comments.New(transport).Create(context.Background(), comments.CreateRequest{ArtworkID: 0, Comment: "body"})
				return err
			},
		},
		{
			...
			run: func(transport *fakeTransport) error {
				_, err := comments.New(transport).Create(context.Background(), comments.CreateRequest{ArtworkID: 1})
				return err
			},
		},
		{
			...
			run: func(transport *fakeTransport) error {
				_, err := comments.New(transport).Reply(context.Background(), comments.ReplyRequest{ArtworkID: 1, Comment: "body"})
				return err
			},
		},
		{
			...
			run: func(transport *fakeTransport) error {
				_, err := comments.New(transport).Stamp(context.Background(), comments.StampRequest{ArtworkID: 1, Comment: "body"})
				return err
			},
		},
		{
...
func TestCommentsCreateRejectsMalformedCommentID(t *testing.T) {
	for _, body := range []string{`{}`, `{"comment_id":null}`, `{"comment_id":0}`, `{"comment_id":-1}`} {
		t.Run(body, func(t *testing.T) {
			...
			result, err := comments.New(transport).Create(context.Background(), comments.CreateRequest{ArtworkID: 1, Comment: "body"})
			if !errors.Is(err, protocol.ErrMalformedResponse) {
...
func TestCommentsMutationsPropagateTransportErrors(t *testing.T) {
	...
	if _, err := comments.New(postJSONTransport).Create(context.Background(), comments.CreateRequest{ArtworkID: 1, Comment: "body"}); !errors.Is(err, wantErr) {
