...
func TestTextHandlerCompactsOperationEvent(t *testing.T) {
	...
	record := slog.NewRecord(time.Date(2026, time.July, 23, 23, 55, 14, 168_000_000, time.FixedZone("CST", 8*60*60)), slog.LevelError, OperationLogMessage, 0)
	record.AddAttrs(
		slog.String(LogFieldComponent, "cli"),
		slog.String(LogFieldOperation, "pixiv"),
		slog.String(LogFieldBackend, BackendLocal),
		slog.Duration(LogFieldDuration, 16500*time.Nanosecond),
		slog.String(LogFieldResult, ResultError),
		slog.String(LogFieldErrorCode, ""),
		slog.Int(LogFieldStatus, 0),
	)
...
func TestTextHandlerIncludesOnlyUsefulOperationMetadata(t *testing.T) {
	...
	record := slog.NewRecord(time.Date(2026, time.July, 23, 23, 55, 14, 0, time.UTC), slog.LevelError, OperationLogMessage, 0)
	record.AddAttrs(
		slog.String(LogFieldComponent, "pixiv_sdk"),
		slog.String(LogFieldOperation, "search_illust"),
		slog.String(LogFieldBackend, "app_api"),
		slog.Duration(LogFieldDuration, 25*time.Millisecond),
		slog.String(LogFieldResult, ResultError),
		slog.String(LogFieldErrorCode, "upstream_error"),
		slog.Int(LogFieldStatus, 502),
		slog.String(LogFieldTransportKind, "timeout"),
		slog.Int64(LogFieldIllustID, 42),
		slog.Int64(LogFieldUserID, 7),
	)
