...
// ID 返回该传输路径的稳定标识符。
func (source ReleaseSource) ID() string { return source.id }

// APIURL 把 canonical GitHub Releases API URL 变换为通过该源访问的地址。
...
// ParseReleaseSources 解析 release_sources.txt 格式的传输路径列表。
