...
func (i *releaseInstaller) Install(ctx context.Context, release Release) (err error) {
	...
	defer func() {
		if err != nil && !filesystem.MustPreserveReplacementSource(err) {
...
func (r releaseFileReplacer) Replace(sourcePath, targetPath string) error {
	if r.goos == "windows" {
		return filesystem.ReplaceFileWithBackup(sourcePath, targetPath, targetPath+".old")
	}
	return filesystem.ReplaceFile(sourcePath, targetPath)
}
