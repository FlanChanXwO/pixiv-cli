...
func TestApplicationServicesReportMissingDependencies(t *testing.T) {
	_, err := config.ConfigService{}.Path()
...
func TestAccountServiceUsesAuthDBAccountStore(t *testing.T) {
	...
	store.listAccounts = func(context.Context) ([]Account, error) {
		callOrder = append(callOrder, "list")
		return []Account{{UserID: 123, Username: "alice", Default: true}}, nil
	}
	store.importAccount = func(_ context.Context, token string, setDefault bool) (Account, error) {
		callOrder = append(callOrder, "import")
		if token != "main/token" {
			t.Fatalf("ImportAccount token=%q", token)
		}
		return Account{UserID: 456, Username: "bob"}, nil
	}
...
func TestAccountServiceImportReportsUpdatedByAuthoritativeReturnedUID(t *testing.T) {
	store := &fakeAccountStore{}
	store.listAccounts = func(context.Context) ([]Account, error) {
		return []Account{{UserID: 456, Username: "before"}}, nil
	}
	store.importAccount = func(context.Context, string, bool) (Account, error) {
		return Account{UserID: 456, Username: "after", Default: true}, nil
	}
...
func TestAccountServiceExportUsesLocalTokens(t *testing.T) {
	store := &fakeAccountStore{}
	store.accountsWithTokens = func(context.Context) ([]AccountWithToken, error) {
		return []AccountWithToken{{UserID: 456, Username: "alice", Default: true, RefreshToken: "opaque-exported-token"}}, nil
	}
...
func TestAccountServiceImportBundleRestoresOffline(t *testing.T) {
	...
	var tokens []string
	store := &fakeAccountStore{}
	store.restoreAccount = func(_ context.Context, account Account, token string, setDefault bool) error {
...
func TestAccountServiceCheckUsesAccountStore(t *testing.T) {
	store := &fakeAccountStore{}
	store.checkAccount = func(_ context.Context, userID int64) (Account, error) {
...
func TestAccountServiceCheckPrefersEnvironmentTokenWithoutSelectingStoredAccount(t *testing.T) {
	...
	store.checkRefreshToken = func(_ context.Context, token string) (Account, error) {
		assert.Equal(t, "environment-token", token)
		return Account{UserID: 456, Username: "environment"}, nil
	}
	service := newAccountServiceForTest(store)
...
func TestAccountServiceCheckExplicitUIDDoesNotUseEnvironmentToken(t *testing.T) {
	...
	store.checkAccount = func(_ context.Context, userID int64) (Account, error) {
		assert.Equal(t, int64(123), userID)
		return Account{UserID: 123, Username: "stored", Default: true}, nil
	}
	store.checkRefreshToken = func(_ context.Context, token string) (Account, error) {
...
func TestAccountServiceRefreshUpdatesPremiumStatus(t *testing.T) {
	premium := false
	store := &fakeAccountStore{}
	store.refreshAccount = func(_ context.Context, userID int64) (Account, error) {
		assert.Equal(t, int64(123), userID)
		return Account{UserID: 123, Username: "refreshed-name", Default: true, Premium: &premium}, nil
	}
...
type fakeAccountStore struct {
	importAccount      func(context.Context, string, bool) (Account, error)
	listAccounts       func(context.Context) ([]Account, error)
	useAccount         func(context.Context, int64) error
	removeAccount      func(context.Context, int64) error
	checkAccount       func(context.Context, int64) (Account, error)
	checkRefreshToken  func(context.Context, string) (Account, error)
	exportRefreshToken func(context.Context, int64) (string, error)
	refreshAccount     func(context.Context, int64) (Account, error)
	currentUser        func(context.Context) (*Account, error)
	restoreAccount     func(context.Context, Account, string, bool) error
	accountsWithTokens func(context.Context) ([]AccountWithToken, error)
}

func (f *fakeAccountStore) ImportAccount(ctx context.Context, token string, setDefault bool) (Account, error) {
	if f.importAccount != nil {
		return f.importAccount(ctx, token, setDefault)
	}
	return Account{}, errors.New("unexpected import")
}

func (f *fakeAccountStore) ListAccounts(ctx context.Context) ([]Account, error) {
...
func (f *fakeAccountStore) CheckAccount(ctx context.Context, userID int64) (Account, error) {
	if f.checkAccount != nil {
		return f.checkAccount(ctx, userID)
	}
	return Account{}, errors.New("unexpected check")
}

func (f *fakeAccountStore) CheckRefreshToken(ctx context.Context, token string) (Account, error) {
	if f.checkRefreshToken != nil {
		return f.checkRefreshToken(ctx, token)
	}
	return Account{}, errors.New("unexpected refresh token check")
}
...
func (f *fakeAccountStore) RefreshAccount(ctx context.Context, userID int64) (Account, error) {
	if f.refreshAccount != nil {
		return f.refreshAccount(ctx, userID)
	}
	return Account{}, errors.New("unexpected refresh")
}

func (f *fakeAccountStore) CurrentUser(ctx context.Context) (*Account, error) {
...
func (f *fakeAccountStore) RestoreAccount(ctx context.Context, account Account, token string, setDefault bool) error {
...
func (f *fakeAccountStore) AccountsWithTokens(ctx context.Context) ([]AccountWithToken, error) {
