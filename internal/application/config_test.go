...
func TestConfigServiceGetSetUnsetDelegateResults(t *testing.T) {
	wantValue := config.SettingValue{Value: true, Text: "true", Source: "file", HasValue: true}
	...
	store := &fakeConfigStore{
		get: func(alias string) (config.SettingValue, error) {
...
func TestConfigServiceGetSetUnsetPropagateDependencyErrors(t *testing.T) {
	...
	service := ConfigService{Store: &fakeConfigStore{
		get:   func(string) (config.SettingValue, error) { return config.SettingValue{}, getErr },
...
type fakeConfigStore struct {
	get   func(string) (config.SettingValue, error)
...
func (f *fakeConfigStore) Get(alias string) (config.SettingValue, error) {
	return f.get(alias)
}
