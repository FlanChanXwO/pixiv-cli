...
// pixivClientAdapter 是 SDKClient 的生产实现：内容方法委托给 *pixiv.Client，
// 账号/认证方法委托给 authdb-backed 的 pixivapp.Service。
type pixivClientAdapter struct {
	client *pixiv.Client
	auth   *pixivapp.Service
}

// NewPixivSDKClient 包装一个已打开的 *pixiv.Client 与账号服务，构造 SDKClient。
func NewPixivSDKClient(client *pixiv.Client, auth *pixivapp.Service) SDKClient {
	return &pixivClientAdapter{client: client, auth: auth}
}

// ---- 账号与认证（委托 pixivapp.Service）----
...
func accountFromPixivApp(account pixivapp.Account) *Account {
