...
func TestRecordFromArtworkPreservesSDKFieldsAndNormalizesID(t *testing.T) {
	...
	require.NoError(t, err)
	want, err := recordpkg.MarshalRecordValue(artwork)
	require.NoError(t, err)
...
func TestRecordFromNovelAndUserPreview(t *testing.T) {
	...
	novelRecord, err := recordpkg.RecordFromNovel(novel)
	require.NoError(t, err)
	assertRecordMatchesSource(t, novel, novelRecord, "88", "novel", "https://www.pixiv.net/novel/show.php?id=88")

	userRecord, err := recordpkg.RecordFromUserPreview(userPreview)
	require.NoError(t, err)
...
func TestRecordFromUserPreviewPreservesCompleteEnvelope(t *testing.T) {
	...
	record, err := recordpkg.RecordFromUserPreview(preview)
	require.NoError(t, err)
...
func TestRecordFromUserDetailPreservesCompleteEnvelope(t *testing.T) {
	...
	record, err := recordpkg.RecordFromUserDetail(detail)
	require.NoError(t, err)
...
func TestRecordMappersRejectNonPositiveSDKID(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "artwork", make: func() (recordpkg.Record, error) {
			return recordpkg.RecordFromArtwork(pixiv.Artwork{ID: 0, Kind: pixiv.ArtworkKindIllustration})
		}},
		{name: "novel", make: func() (recordpkg.Record, error) {
			return recordpkg.RecordFromNovel(pixiv.Novel{ID: -1})
		}},
		{name: "user", make: func() (recordpkg.Record, error) {
			return recordpkg.RecordFromUserPreview(pixiv.UserPreview{User: pixiv.User{ID: 0}})
		}},
		{name: "user detail", make: func() (recordpkg.Record, error) {
			return recordpkg.RecordFromUserDetail(pixiv.UserDetail{User: pixiv.User{ID: 0}})
		}},
	} {
...
func TestRecordMatchesLeavesUnfilteredRecordsUnchangedAndRejectsMissingMetrics(t *testing.T) {
	...
	assert.True(t, record.Matches(recordpkg.RecordFilter{}))
	assert.True(t, record.Matches(recordpkg.RecordFilter{ID: "user-1", Type: "user"}))
	assert.False(t, record.Matches(recordpkg.RecordFilter{ID: "other"}))
	assert.False(t, record.Matches(recordpkg.RecordFilter{Type: "illust"}))
	assert.False(t, record.Matches(recordpkg.RecordFilter{Tags: []string{"tag-a"}}))
	assert.False(t, record.Matches(recordpkg.RecordFilter{MinViews: int64Ptr(0)}))
	assert.False(t, record.Matches(recordpkg.RecordFilter{MinPageCount: int64Ptr(0)}))
}
...
func assertRecordMatchesSource(t *testing.T, source any, record recordpkg.Record, id, recordType, url string) {
	...
	got, err := json.Marshal(record)
	require.NoError(t, err)
	want, err := recordpkg.MarshalRecordValue(source)
	require.NoError(t, err)
