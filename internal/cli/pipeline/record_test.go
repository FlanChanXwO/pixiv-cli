...
func TestRecordFromNovelAndUserPreview(t *testing.T) {
	...
	novelRecord, err := pipeline.RecordFromNovelDTO(pixiv.ToNovelDTO(novel))
	require.NoError(t, err)
	assertRecordMatchesSource(t, novel, novelRecord, "88", "novel", "https://www.pixiv.net/novel/show.php?id=88")

	userRecord, err := pipeline.RecordFromUserPreviewDTO(pixiv.ToUserPreviewDTO(userPreview))
	require.NoError(t, err)
...
func TestRecordFromUserPreviewPreservesCompleteEnvelope(t *testing.T) {
	...
	record, err := pipeline.RecordFromUserPreviewDTO(pixiv.ToUserPreviewDTO(preview))
	require.NoError(t, err)
...
func TestRecordFromUserDetailPreservesCompleteEnvelope(t *testing.T) {
	...
	record, err := pipeline.RecordFromUserDetailDTO(pixiv.ToUserDetailDTO(detail))
	require.NoError(t, err)
...
func TestRecordMappersRejectNonPositiveSDKID(t *testing.T) {
	for _, test := range []struct {
		name string
		make func() (pipeline.Record, error)
	}{
		{name: "artwork", make: func() (pipeline.Record, error) {
			return pipeline.RecordFromArtworkDTO(pixiv.ToArtworkDTO(pixiv.Artwork{ID: 0, Kind: pixiv.ArtworkKindIllustration}))
		}},
		{name: "novel", make: func() (pipeline.Record, error) {
			return pipeline.RecordFromNovelDTO(pixiv.ToNovelDTO(pixiv.Novel{ID: -1}))
		}},
		{name: "user", make: func() (pipeline.Record, error) {
			return pipeline.RecordFromUserPreviewDTO(pixiv.ToUserPreviewDTO(pixiv.UserPreview{User: pixiv.User{ID: 0}}))
		}},
		{name: "user detail", make: func() (pipeline.Record, error) {
			return pipeline.RecordFromUserDetailDTO(pixiv.ToUserDetailDTO(pixiv.UserDetail{User: pixiv.User{ID: 0}}))
		}},
	} {
...
func TestParseRecordJSONNormalizesNumericIDAndPreservesUnknownFields(t *testing.T) {
	record, err := pipeline.ParseRecordJSON([]byte(`{
		"id":9007199254740993,
		"type":"illust",
...
func TestParseRecordJSONRejectsInvalidRequiredFields(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			_, err := pipeline.ParseRecordJSON([]byte(test.line))
			require.Error(t, err)
...
func TestRecordMatchesFiltersForObjectAndStringTags(t *testing.T) {
	objectTags, err := pipeline.ParseRecordJSON([]byte(`{
		"id":"7",
		"type":"illust",
	...
	require.NoError(t, err)
	stringTags, err := pipeline.ParseRecordJSON([]byte(`{
		"id":"8",
		"type":"novel",
	...
	assert.False(t, objectTags.Matches(pipeline.RecordFilter{Tags: []string{"tag-a", "missing"}}))
	assert.True(t, stringTags.Matches(pipeline.RecordFilter{Tags: []string{"tag-a", "tag-b"}, MinViews: int64Ptr(100), MinPageCount: int64Ptr(1)}))
	assert.False(t, stringTags.Matches(pipeline.RecordFilter{MinViews: int64Ptr(101)}))
	assert.False(t, stringTags.Matches(pipeline.RecordFilter{MinPageCount: int64Ptr(2)}))
}
...
func TestRecordMatchesLeavesUnfilteredRecordsUnchangedAndRejectsMissingMetrics(t *testing.T) {
	...
	assert.True(t, record.Matches(pipeline.RecordFilter{}))
	assert.True(t, record.Matches(pipeline.RecordFilter{ID: "user-1", Type: "user"}))
	assert.False(t, record.Matches(pipeline.RecordFilter{ID: "other"}))
	assert.False(t, record.Matches(pipeline.RecordFilter{Type: "illust"}))
	assert.False(t, record.Matches(pipeline.RecordFilter{Tags: []string{"tag-a"}}))
	assert.False(t, record.Matches(pipeline.RecordFilter{MinViews: int64Ptr(0)}))
	assert.False(t, record.Matches(pipeline.RecordFilter{MinPageCount: int64Ptr(0)}))
}

func TestRecordMatchesTreatsNullAndWrongTypesAsMissingFields(t *testing.T) {
	nullTags, err := pipeline.ParseRecordJSON([]byte(`{
		"id":"tags-null","type":"illust","url":"https://www.pixiv.net/artworks/1","tags":[null]
	}`))
	require.NoError(t, err)
	nullViews, err := pipeline.ParseRecordJSON([]byte(`{
		"id":"views-null","type":"illust","url":"https://www.pixiv.net/artworks/2","total_view":null,"views":7
	}`))
	require.NoError(t, err)
	wrongTypeViews, err := pipeline.ParseRecordJSON([]byte(`{
		"id":"views-wrong","type":"illust","url":"https://www.pixiv.net/artworks/3","total_view":"unknown","views":7
	}`))
	require.NoError(t, err)
	nullPages, err := pipeline.ParseRecordJSON([]byte(`{
		"id":"pages-null","type":"illust","url":"https://www.pixiv.net/artworks/4","page_count":null
	}`))
	require.NoError(t, err)
	wrongTypePages, err := pipeline.ParseRecordJSON([]byte(`{
		"id":"pages-wrong","type":"illust","url":"https://www.pixiv.net/artworks/5","page_count":"one"
	}`))
	require.NoError(t, err)

	assert.False(t, nullTags.Matches(pipeline.RecordFilter{Tags: []string{""}}))
	assert.True(t, nullViews.Matches(pipeline.RecordFilter{MinViews: int64Ptr(7)}))
	assert.True(t, wrongTypeViews.Matches(pipeline.RecordFilter{MinViews: int64Ptr(7)}))
	assert.False(t, nullPages.Matches(pipeline.RecordFilter{MinPageCount: int64Ptr(0)}))
	assert.False(t, wrongTypePages.Matches(pipeline.RecordFilter{MinPageCount: int64Ptr(0)}))
}
...
func TestRecordMatchesSupportsInt64MetricThresholds(t *testing.T) {
	...
	require.NoError(t, err)
	below, err := pipeline.ParseRecordJSON([]byte(`{
	...
	filter := pipeline.RecordFilter{MinViews: &max, MinPageCount: &max}
	assert.True(t, record.Matches(filter))
	assert.False(t, below.Matches(filter))
}

func assertRecordMatchesSource(t *testing.T, source any, record pipeline.Record, id, recordType, url string) {
	t.Helper()
	got, err := json.Marshal(record)
	require.NoError(t, err)
