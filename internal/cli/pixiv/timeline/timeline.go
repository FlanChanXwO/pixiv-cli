...
func (a command) runFollowing(cmd *cobra.Command, opts options) error {
	...
	if entity == "artwork" {
		...
		return a.runner().RunPooledIllustList(cmd.Context(), request, plan, jsonOut, ndjson, "new artworks from followed users", fetch,
	...
	return a.runner().RunPooledNovelList(cmd.Context(), request, plan, jsonOut, ndjson, "new novels from followed users", fetch,
...
func (a command) runLatest(cmd *cobra.Command, opts options) error {
	...
	if entity == "novel" {
		...
		return a.runner().RunPooledNovelList(cmd.Context(), request, plan, jsonOut, ndjson, "latest novels", fetch,
			func(items []pixiv.Novel) error { return printNovels(a.data.Output, items) })
	}
	fetch := func(client *pixiv.Client, ctx context.Context, cursor sdk.Cursor) ([]pixiv.Artwork, sdk.Cursor, error) {
	...
	return a.runner().RunPooledIllustList(cmd.Context(), request, plan, jsonOut, ndjson, fmt.Sprintf("latest %s", opts.artworkType), fetch,
		func(items []pixiv.Artwork, start int) error { return printArtworks(a.data.Output, items) })
}
