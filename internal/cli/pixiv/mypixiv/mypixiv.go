...
func (a command) runUsers(cmd *cobra.Command, opts options) error {
	...
	return a.runner().RunPooledUserList(cmd.Context(), request, plan, jsonOut, opts.ndjson, func() string {
...
func (a command) runWorks(cmd *cobra.Command, args []string, opts options) error {
	...
	if len(args) == 0 {
		if opts.contentType == "illust" {
			...
			return a.runner().RunPooledIllustList(cmd.Context(), request, plan, jsonOut, ndjson, "MyPixiv artworks", fetch,
		...
		return a.runner().RunPooledNovelList(cmd.Context(), request, plan, jsonOut, ndjson, "MyPixiv novels", fetch,
	...
	return a.runner().RunPooledIllustList(cmd.Context(), request, plan, jsonOut, ndjson, fmt.Sprintf("artworks by %d", userID), fetch,
		func(items []pixiv.Artwork, start int) error { return printArtworks(a.data.Output, items) })
}
