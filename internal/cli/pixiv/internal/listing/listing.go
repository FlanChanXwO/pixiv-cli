...
// Runner executes one paged operation with the SDK ports selected by a command
...
func New(out io.Writer, deps pixivdeps.Data) Runner {
	return Runner{out: out, pooled: deps.Pooled}
}

// PooledOperation 只把当前 CLI 请求接入 session 提供的重放边界；逻辑分页、
// bookmark 策略和 cursor 处理由 internal/search/pixiv 拥有。
func (a Runner) PooledOperation(request pixivdeps.Request) searchpixiv.Operation[*pixiv.Client] {
	if a.pooled == nil {
		return nil
	}
	return func(ctx context.Context, attempt func(context.Context, *pixiv.Client) (bool, error)) error {
		return a.pooled(ctx, request, attempt)
	}
}
...
func (a Runner) RunPooledIllustList(ctx context.Context, request pixivdeps.Request, plan Plan, jsonOut, ndjson bool, heading string, fetch func(*pixiv.Client, context.Context, sdk.Cursor) ([]pixiv.Artwork, sdk.Cursor, error), print func([]pixiv.Artwork, int) error) error {
...
func (a Runner) RunPooledIllustListWithHeading(ctx context.Context, request pixivdeps.Request, plan Plan, jsonOut, ndjson bool, heading func() string, fetch func(*pixiv.Client, context.Context, sdk.Cursor) ([]pixiv.Artwork, sdk.Cursor, error), print func([]pixiv.Artwork, int) error) error {
...
// 账号池提交边界。
func (a Runner) RunPooledIllustListWithKey(ctx context.Context, request pixivdeps.Request, plan Plan, jsonOut, ndjson bool, jsonKey string, heading func() string, fetch func(*pixiv.Client, context.Context, sdk.Cursor) ([]pixiv.Artwork, sdk.Cursor, error), print func([]pixiv.Artwork, int) error) error {
	...
	err := searchpixiv.RunPooledPagedRead(ctx, a.PooledOperation(request), plan.PagePlan(), begin,
...
func (a Runner) RunPooledNovelList(ctx context.Context, request pixivdeps.Request, plan Plan, jsonOut, ndjson bool, heading string, fetch func(*pixiv.Client, context.Context, sdk.Cursor) ([]pixiv.Novel, sdk.Cursor, error), print func([]pixiv.Novel) error) error {
	var spool *jsonArraySpool
	if jsonOut {
		defer func() {
			if spool != nil {
...
// 可安全重放的边界内。
func (a Runner) RunPooledUserList(ctx context.Context, request pixivdeps.Request, plan Plan, jsonOut, ndjson bool, heading func() string, fetch func(*pixiv.Client, context.Context, sdk.Cursor) ([]pixiv.UserPreview, sdk.Cursor, error), print func([]pixiv.UserPreview) error) error {
	var spool *jsonArraySpool
	if jsonOut {
		defer func() {
			if spool != nil {
