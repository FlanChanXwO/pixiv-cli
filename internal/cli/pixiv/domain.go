...
func (a controller) newUserCommand() *cobra.Command {
...
func (a controller) newUserSearchCommand() *cobra.Command {
	opts := userSearchOptions{}
	cmd := &cobra.Command{Use: "search WORD", Short: "Search users", Example: "pixiv user search \"miku\" --json", Args: a.requireMinArgs(1, "pixiv user search [options] WORD"), RunE: func(cmd *cobra.Command, args []string) error {
...
func (a controller) runUserSearch(cmd *cobra.Command, args []string, opts userSearchOptions) error {
	...
	return services.SDK.RunPooledOperation(cmd.Context(), request, func(ctx context.Context, client pixivapp.ClientSet) (bool, error) {
		...
		err := pageItems(ctx, plan, fetch, func(items []pixiv.UserPreview) error {
			if !headingWritten {
				committed = true
				if _, err := fmt.Fprintf(a.out, "users for %q\n", word); err != nil {
					return err
				}
				headingWritten = true
			}
...
func printUserSearchPreviews(out io.Writer, items []pixiv.UserPreview) error {
	for _, item := range items {
		line := fmt.Sprintf("%d %s", item.User.ID, clioutput.SafeTextLine(item.User.Name))
		if item.User.Account != "" {
			line += " (@" + clioutput.SafeTextLine(item.User.Account) + ")"
		}
		if item.User.Comment != "" {
			line += " — " + clioutput.SafeTextLine(item.User.Comment)
		}
...
func (a controller) newUserDetailCommand() *cobra.Command {
	var opts commandOptions
	cmd := &cobra.Command{Use: "detail USER_ID", Short: "Show one user's complete profile", Args: a.requireExactArgs(1, "pixiv user detail [options] USER_ID"), RunE: func(cmd *cobra.Command, args []string) error {
...
func (a controller) runUserDetail(cmd *cobra.Command, arg string, opts commandOptions) error {
...
func (a controller) newUserArtworksCommand() *cobra.Command {
	opts := userListOptions{illustType: string(pixiv.ArtworkKindIllustration)}
	cmd := &cobra.Command{Use: "artworks [USER_ID]", Short: "List a user's artworks", Args: a.requireMaxArgs(1, "pixiv user artworks [options] [USER_ID]"), RunE: func(cmd *cobra.Command, args []string) error {
...
func (a controller) newUserBookmarksCommand() *cobra.Command {
	opts := userListOptions{restrict: string(pixiv.RestrictPublic)}
	cmd := &cobra.Command{Use: "bookmarks [USER_ID]", Short: "List a user's bookmarks", Args: a.requireMaxArgs(1, "pixiv user bookmarks [options] [USER_ID]"), RunE: func(cmd *cobra.Command, args []string) error {
...
func (a controller) newUserFollowingCommand() *cobra.Command {
	opts := userListOptions{restrict: string(pixiv.RestrictPublic)}
	cmd := &cobra.Command{Use: "following [USER_ID]", Short: "List users followed by a user", Args: a.requireMaxArgs(1, "pixiv user following [options] [USER_ID]"), RunE: func(cmd *cobra.Command, args []string) error {
...
func (a controller) runUserArtworks(cmd *cobra.Command, args []string, options userListOptions) error {
...
func (a controller) runUserBookmarks(cmd *cobra.Command, args []string, options userListOptions) error {
	plan, err := parseListPlan(cmd, options.listOptions)
	if err != nil {
		return err
	}
...
func (a controller) runUserFollowing(cmd *cobra.Command, args []string, options userListOptions) error {
	plan, err := parseListPlan(cmd, options.listOptions)
	if err != nil {
		return err
	}
...
func (a controller) newBookmarkCommand() *cobra.Command {
...
func (a controller) newBookmarkAddCommand() *cobra.Command {
	opts := mutationOptions{restrict: string(pixiv.RestrictPublic)}
	cmd := &cobra.Command{Use: "add [ILLUST_ID]", Short: "Bookmark an illustration", Args: a.actionInputArgs("pixiv bookmark add [options] [ILLUST_ID]"), RunE: func(cmd *cobra.Command, args []string) error {
		if _, err := clioutput.RecordFailureStrategy(opts.onError); err != nil {
			return a.usageError(err)
		}
		invoke := a.lazyActionInvoker(cmd, opts.commandOptions, func(client pixivapp.ClientSet, ctx context.Context, id int64) error {
...
func (a controller) newBookmarkRemoveCommand() *cobra.Command {
	opts := mutationOptions{}
	cmd := &cobra.Command{Use: "remove [ILLUST_ID]", Short: "Remove an illustration bookmark", Args: a.actionInputArgs("pixiv bookmark remove [options] [ILLUST_ID]"), RunE: func(cmd *cobra.Command, args []string) error {
		if _, err := clioutput.RecordFailureStrategy(opts.onError); err != nil {
			return a.usageError(err)
		}
		invoke := a.lazyActionInvoker(cmd, opts.commandOptions, func(client pixivapp.ClientSet, ctx context.Context, id int64) error {
...
func (a controller) newFollowCommand() *cobra.Command {
	cmd := &cobra.Command{Use: "follow", Short: "Manage followed users"}
	cmd.AddCommand(a.newFollowAddCommand(), a.newFollowRemoveCommand())
	return cmd
}

func (a controller) newFollowAddCommand() *cobra.Command {
	opts := mutationOptions{restrict: string(pixiv.RestrictPublic)}
	cmd := &cobra.Command{Use: "add [USER_ID]", Short: "Follow a user", Args: a.actionInputArgs("pixiv follow add [options] [USER_ID]"), RunE: func(cmd *cobra.Command, args []string) error {
		if _, err := clioutput.RecordFailureStrategy(opts.onError); err != nil {
			return a.usageError(err)
		}
		invoke := a.lazyActionInvoker(cmd, opts.commandOptions, func(client pixivapp.ClientSet, ctx context.Context, id int64) error {
...
func (a controller) newFollowRemoveCommand() *cobra.Command {
	opts := mutationOptions{}
	cmd := &cobra.Command{Use: "remove [USER_ID]", Short: "Unfollow a user", Args: a.actionInputArgs("pixiv follow remove [options] [USER_ID]"), RunE: func(cmd *cobra.Command, args []string) error {
		if _, err := clioutput.RecordFailureStrategy(opts.onError); err != nil {
			return a.usageError(err)
		}
		invoke := a.lazyActionInvoker(cmd, opts.commandOptions, func(client pixivapp.ClientSet, ctx context.Context, id int64) error {
...
func (a controller) lazyActionInvoker(cmd *cobra.Command, options commandOptions, invoke func(pixivapp.ClientSet, context.Context, int64) error) func(context.Context, int64) error {
	var client pixivapp.ClientSet
