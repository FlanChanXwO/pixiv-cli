...
func ConsumeNDJSONRecords(ctx context.Context, in io.Reader, errOut io.Writer, operation string, failFast bool, consume func(context.Context, recordpkg.Record) error) error {
	...
	for {
		...
		if len(line) > 0 {
			lineNumber++
			record, recordErr := recordpkg.ParseRecordJSON(bytes.TrimSpace(line))
			if recordErr != nil {
				failed = true
...
func RequiredRecordID(record recordpkg.Record) (int64, error) {
