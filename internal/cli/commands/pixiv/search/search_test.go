...
func TestCommandRejectsRatingFilterBeforeSearch(t *testing.T) {
	opened := false
	cmd := New(Dependencies{
	...
	cmd.SetArgs([]string{"cat", "--rating", "r18", "--page", "2", "--limit", "1"})
	if err := cmd.Execute(); err == nil || !strings.Contains(err.Error(), "rating filtering is not supported") {
		t.Fatalf("rating error = %v", err)
	}
	if opened {
		t.Fatal("unsupported rating reached upstream search")
	}
}
