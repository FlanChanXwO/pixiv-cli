...
func TestDownloadRecordTypesConsumeArtworkByID(t *testing.T) {
	...
	for _, recordType := range []string{"artwork", "illust", "manga", "ugoira"} {
		t.Run(recordType, func(t *testing.T) {
			var gotID int64
			err := pipeline.ConsumeActionRecords(
				context.Background(),
				strings.NewReader(fmt.Sprintf(`{"id":"42","type":%q,"url":"https://www.pixiv.net/artworks/42"}`+"\n", recordType)),
				&bytes.Buffer{}, "download", "fail-fast", visualRecordTypes,
...
func TestDownloadRecordTypesStillRejectNonArtworkEntities(t *testing.T) {
	for _, recordType := range []string{"user", "novel"} {
		t.Run(recordType, func(t *testing.T) {
			...
			err := pipeline.ConsumeActionRecords(
				...
				&diagnostics, "download", "fail-fast", visualRecordTypes,
