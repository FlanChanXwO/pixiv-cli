...
func pixivURLHandlerAppPath() (string, error) {
	appDataDir, err := filesystem.UserDataSubdir(filesystem.AppDataDirName)
	if err != nil {
		return "", err
	}
...
func saveHandlerBundleManifest(appPath string, manifest handlerManifest) error {
	...
	return filesystem.WritePrivateFile(handlerBundleManifestPath(appPath), body, filesystem.PrivateFileMode)
}
...
func ensurePixivURLHandlerAppWithCompiler(ctx context.Context, appPath string, compile pixivURLHandlerCompiler) error {
	...
	if err := os.MkdirAll(filepath.Dir(executablePath), filesystem.PrivateDirMode); err != nil {
		return err
	}
	...
	if err := os.Chmod(sourceDir, filesystem.PrivateDirMode); err != nil {
		return err
	}
	...
	if err := source.Chmod(filesystem.PrivateFileMode); err != nil {
		_ = source.Close()
		return err
	}
	...
	if err := os.WriteFile(infoPath, []byte(pixivURLHandlerInfoPlist), filesystem.PrivateFileMode); err != nil {
		return err
	}
	if err := filesystem.WritePrivateFile(versionPath, []byte(pixivURLHandlerSourceVersion+"\n"), filesystem.PrivateFileMode); err != nil {
		return err
	}
	return nil
}
