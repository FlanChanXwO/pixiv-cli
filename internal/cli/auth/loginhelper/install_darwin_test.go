...
func TestEnsurePixivURLHandlerAppDoesNotFollowLegacyTemporarySourceSymlink(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(canaryPath, []byte("do not overwrite"), filesystem.PrivateFileMode))
...
func TestPixivURLHandlerAppPathUsesApplicationDataDirectory(t *testing.T) {
	...
	require.Equal(t, filepath.Join(home, filesystem.AppDataDirName, "url-handler", "PixivCLIURLHandler.app"), path)
}
...
func TestEnsurePixivURLHandlerAppCompilesPrivateRandomSourceAndCleansIt(t *testing.T) {
	...
	compile := func(_ context.Context, actualSourcePath, executablePath string) ([]byte, error) {
		...
		require.Equal(t, os.FileMode(filesystem.PrivateDirMode), dirInfo.Mode().Perm())

		sourceInfo, err := os.Lstat(actualSourcePath)
		require.NoError(t, err)
		require.True(t, sourceInfo.Mode().IsRegular())
		require.Equal(t, os.FileMode(filesystem.PrivateFileMode), sourceInfo.Mode().Perm())
		content, err := os.ReadFile(actualSourcePath)
		require.NoError(t, err)
		require.Equal(t, pixivURLHandlerSwiftSource, string(content))

		require.NoError(t, os.WriteFile(executablePath, []byte("compiled"), filesystem.PrivateFileMode))
		return nil, nil
	}
...
func TestEnsurePixivURLHandlerAppRebuildsHelperWithoutCurrentSourceVersion(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(filepath.Dir(executablePath), filesystem.PrivateDirMode))
	require.NoError(t, os.WriteFile(executablePath, []byte("old helper"), filesystem.PrivateFileMode))
	require.NoError(t, os.WriteFile(infoPath, []byte("old metadata"), filesystem.PrivateFileMode))

	compiled := false
	compile := func(_ context.Context, _ string, executable string) ([]byte, error) {
		compiled = true
		require.NoError(t, os.WriteFile(executable, []byte("new helper"), filesystem.PrivateFileMode))
		return nil, nil
	}
...
func TestPixivURLHandlerReadsPrivateManifest(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, filepath.Join(home, filesystem.AppDataDirName, "url-handler", handlerManifestFilename), manifest)
	require.Contains(t, pixivURLHandlerSwiftSource, "NSHomeDirectory()")
	require.Contains(t, pixivURLHandlerSwiftSource, filesystem.AppDataDirName+"/url-handler/"+handlerManifestFilename)
...
func TestPixivURLHandlerStoresLaunchManifestInItsOwnBundle(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, os.FileMode(filesystem.PrivateFileMode), info.Mode().Perm())
...
func TestPixivURLHandlerSwiftSourceCompiles(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(sourcePath, []byte(pixivURLHandlerSwiftSource), filesystem.PrivateFileMode))
