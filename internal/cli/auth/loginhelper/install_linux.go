...
func EnsurePersistent(ctx context.Context) error {
	...
	if err := filesystem.WritePrivateFile(desktopPath, []byte(linuxDesktopEntry(executablePath)), filesystem.PrivateFileMode); err != nil {
		return err
	}
...
func restoreLinuxMimeState(snapshots []linuxFileSnapshot) error {
	...
	for _, snapshot := range snapshots {
		...
		if err := os.MkdirAll(filepath.Dir(snapshot.Path), filesystem.PrivateDirMode); err != nil {
			errs = append(errs, err)
			continue
		}
