...
func TestWriteManualEscapesLoginURL(t *testing.T) {
	...
	err := WriteManual(&out, "https://app-api.pixiv.net/web/v1/login?state=one&code_challenge=two")
	if err != nil {
...
func TestWriteCallbackRelayContainsLocalPOSTBridge(t *testing.T) {
	...
	var out bytes.Buffer
	if err := WriteCallbackRelay(&out); err != nil {
...
func TestWriteResultKeepsFailureGeneric(t *testing.T) {
	...
	if err := WriteResult(&success, true); err != nil {
	...
	if err := WriteResult(&failure, false); err != nil {
...
func TestPagesDoNotOverrideBrowserFavicon(t *testing.T) {
	...
	if err := WriteManual(&manual, "https://app-api.pixiv.net/web/v1/login?state=test"); err != nil {
		t.Fatalf("WriteManual() error = %v", err)
	}
	if err := WriteCallbackRelay(&callback); err != nil {
		t.Fatalf("WriteCallbackRelay() error = %v", err)
	}
	if err := WriteResult(&success, true); err != nil {
		t.Fatalf("WriteResult(success) error = %v", err)
