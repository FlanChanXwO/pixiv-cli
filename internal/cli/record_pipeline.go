...
func consumeNDJSONRecords(ctx context.Context, in io.Reader, errOut io.Writer, operation string, failFast bool, consume func(context.Context, application.Record) error) error {
	...
	for {
		...
		if len(line) > 0 {
			lineNumber++
			record, recordErr := application.ParseRecordJSON(bytes.TrimSpace(line))
			if recordErr != nil {
				failed = true
...
func recordFailureStrategy(value string) (bool, error) {
	...
	case "fail-fast":
		return true, nil
	default:
		return false, newUsageError(errors.New("on-error must be one of: skip, fail-fast"))
	}
}
...
func requiredRecordID(record application.Record) (int64, error) {
