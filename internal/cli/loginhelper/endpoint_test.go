...
func TestCallbackRelayURLUsesPrivateLoopbackEndpointAndFragment(t *testing.T) {
	...
	wantPath, err := callbackEndpointPath()
	...
	relay, err := CallbackRelayURL("pixiv://account/login?code=one-time-code")
	require.NoError(t, err)
...
func TestCallbackRelayURLRejectsNonLoopbackAndInactiveEndpoints(t *testing.T) {
	...
	_, err = CallbackRelayURL("pixiv://account/login?code=one-time-code")
	...
	_, err = CallbackRelayURL("https://example.invalid/callback?code=one-time-code")
