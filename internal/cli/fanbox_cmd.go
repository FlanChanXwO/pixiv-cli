...
// fanboxBrowserProvider 读取浏览器中的 FANBOXSESSID。browsercookies 包尚未
// 落地时默认实现返回明确错误；落地后由内部实现替换。CLI 测试可注入 fake。
type fanboxBrowserProvider interface {
	ReadSession(ctx context.Context, browser, profileID string) (string, error)
}

var fanboxBrowserSessionReader fanboxBrowserProvider = unavailableFanboxBrowserSessionReader{}

type unavailableFanboxBrowserSessionReader struct{}

func (unavailableFanboxBrowserSessionReader) ReadSession(context.Context, string, string) (string, error) {
	return "", errors.New("browser session import is not available in this build")
}
...
func (a app) newFanboxCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Args:  requireExactArgs(0, "pixiv fanbox <command>"),
...
func (a app) fanboxService() (*fanboxapp.Service, error) {
	services := a.services()
	if services.Fanbox == nil {
		return nil, errors.New("fanbox is not available: cannot open the local account store")
	}
	return services.Fanbox, nil
}

func (a app) fanboxClient(ctx context.Context) (*fanbox.Client, error) {
	service, err := a.fanboxService()
	if err != nil {
		return nil, err
	}
	return service.OpenClient(ctx)
}
...
func (a app) newFanboxAuthCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage local FANBOX authentication",
		Args:  requireExactArgs(0, "pixiv fanbox auth <command>"),
		RunE: func(cmd *cobra.Command, args []string) error {
			return cmd.Help()
		},
	}
...
func (a app) newFanboxAuthImportCommand() *cobra.Command {
	opts := fanboxAuthImportOptions{}
	cmd := &cobra.Command{
		Use:     "import",
		Short:   "Import a FANBOX session",
		Example: "pixiv fanbox auth import --stdin",
		Args:    requireExactArgs(0, "pixiv fanbox auth import --stdin | --from-browser BROWSER [--profile ID] [--default]"),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func (a app) fanboxAuthImport(cmd *cobra.Command, opts fanboxAuthImportOptions) error {
	if opts.stdin == (opts.fromBrowser != "") {
		return errors.New("use exactly one of --stdin or --from-browser")
	}
	if opts.fromBrowser != "" {
		value, err := fanboxBrowserSessionReader.ReadSession(cmd.Context(), opts.fromBrowser, opts.profile)
		if err != nil {
			return err
		}
...
func (a app) fanboxImportSession(cmd *cobra.Command, value string, setDefault, jsonOut bool) error {
	service, err := a.fanboxService()
	if err != nil {
		return err
	}
	account, err := service.ImportSession(cmd.Context(), value, setDefault)
	if err != nil {
		return err
	}
	if jsonOut {
...
func (a app) newFanboxAuthListCommand() *cobra.Command {
	var jsonOut bool
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List FANBOX accounts",
		Args:  requireExactArgs(0, "pixiv fanbox auth list [--json]"),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func (a app) fanboxAuthList(jsonOut bool) error {
	service, err := a.fanboxService()
	if err != nil {
		return err
	}
...
func (a app) newFanboxAuthUseCommand() *cobra.Command {
	opts := fanboxAuthUseOptions{}
	cmd := &cobra.Command{
		Use:   "use [UID]",
		Short: "Set the default FANBOX account",
		Args:  requireMaxArgs(1, "pixiv fanbox auth use [UID] | --auto"),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func (a app) fanboxAuthUse(args []string, opts fanboxAuthUseOptions) error {
...
func (a app) newFanboxAuthRemoveCommand() *cobra.Command {
	opts := fanboxAuthRemoveOptions{}
	cmd := &cobra.Command{
		Use:   "remove UID",
		Short: "Remove a FANBOX account",
		Args:  requireExactArgs(1, "pixiv fanbox auth remove UID"),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func (a app) fanboxAuthRemove(args []string, opts fanboxAuthRemoveOptions) error {
	userID, err := parseFanboxUID(args[0])
	if err != nil {
		return err
	}
	...
	if canPrompt(a) && !opts.yes {
		ok, err := promptConfirm(a, fmt.Sprintf("Remove fanbox uid %d?", userID), false)
		if err != nil {
			return err
		}
		if !ok {
...
func (a app) newFanboxAuthStatusCommand() *cobra.Command {
	var jsonOut bool
	cmd := &cobra.Command{
		Use:   "status [UID]",
		Short: "Show the default FANBOX account",
		Args:  requireMaxArgs(1, "pixiv fanbox auth status [UID]"),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func (a app) fanboxAuthStatus(args []string, jsonOut bool) error {
	service, err := a.fanboxService()
	if err != nil {
		return err
	}
...
func (a app) printFanboxAccountSummary(summary fanboxapp.AccountSummary, jsonOut bool) error {
	if jsonOut {
...
func (a app) fanboxJSONOut(cmd *cobra.Command, opts fanboxListOptions) (bool, error) {
	if opts.ndjson && cmd.Flags().Changed("json") {
		return false, newUsageError(fmt.Errorf("--ndjson cannot be used with --json"))
	}
	return !opts.ndjson && opts.jsonOut, nil
}

func (a app) newFanboxCreatorsCommand() *cobra.Command {
	opts := fanboxListOptions{kind: string(fanbox.CreatorListSupporting)}
	cmd := &cobra.Command{
		Use:   "creators",
		Short: "List supporting or following FANBOX creators",
		Args:  requireExactArgs(0, "pixiv fanbox creators --kind supporting|following"),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func (a app) fanboxCreators(cmd *cobra.Command, opts fanboxListOptions) error {
	...
	client, err := a.fanboxClient(cmd.Context())
	if err != nil {
		return err
	}
...
func (a app) newFanboxPostsCommand() *cobra.Command {
	opts := fanboxListOptions{}
	cmd := &cobra.Command{
		Use:   "posts SOURCE",
		Short: "List posts from a creator, tag, post, or FANBOX URL",
		Args:  requireExactArgs(1, "pixiv fanbox posts SOURCE"),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func (a app) fanboxPosts(cmd *cobra.Command, args []string, opts fanboxListOptions) error {
	plan, err := parseListPlan(cmd, opts.listOptions)
	if err != nil {
		return err
	}
...
func (a app) fanboxPostsFetch(ctx context.Context, client *fanbox.Client, source string) (func(context.Context, sdk.Cursor) (sdk.Page[fanbox.Post], error), error) {
	switch {
...
func (a app) newFanboxTagsCommand() *cobra.Command {
	opts := fanboxListOptions{}
	cmd := &cobra.Command{
		Use:   "tags CREATOR",
		Short: "List tags used by a FANBOX creator",
		Args:  requireExactArgs(1, "pixiv fanbox tags CREATOR"),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func (a app) fanboxTags(cmd *cobra.Command, args []string, opts fanboxListOptions) error {
	jsonOut, err := a.fanboxJSONOut(cmd, opts)
	if err != nil {
		return err
	}
...
func (a app) newFanboxHomeCommand() *cobra.Command {
	opts := fanboxListOptions{}
	cmd := &cobra.Command{
		Use:   "home",
		Short: "Browse the FANBOX home feed",
		Args:  requireExactArgs(0, "pixiv fanbox home"),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func (a app) fanboxHome(cmd *cobra.Command, opts fanboxListOptions) error {
	plan, err := parseListPlan(cmd, opts.listOptions)
	if err != nil {
		return err
	}
...
func (a app) newFanboxSupportingCommand() *cobra.Command {
	opts := fanboxListOptions{}
	cmd := &cobra.Command{
		Use:   "supporting",
		Short: "Browse posts from supporting creators",
		Args:  requireExactArgs(0, "pixiv fanbox supporting"),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func (a app) fanboxSupporting(cmd *cobra.Command, opts fanboxListOptions) error {
	plan, err := parseListPlan(cmd, opts.listOptions)
	if err != nil {
		return err
	}
...
func (a app) newFanboxPostCommand() *cobra.Command {
	opts := fanboxListOptions{}
	cmd := &cobra.Command{
		Use:   "post POST_ID",
		Short: "Show one FANBOX post",
		Args:  requireExactArgs(1, "pixiv fanbox post POST_ID"),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func (a app) fanboxPost(cmd *cobra.Command, args []string, opts fanboxListOptions) error {
	jsonOut, err := a.fanboxJSONOut(cmd, opts)
	if err != nil {
		return err
	}
...
func printFanboxPosts(out io.Writer, posts []fanbox.Post) error {
	for _, post := range posts {
		fmt.Fprintf(out, "id:%s title:%s published:%s restricted:%s\n",
			post.ID, post.Title, fanboxPostOutFrom(post).PublishedAt, textBool(post.IsRestricted))
	}
	return nil
}
...
func (a app) newFanboxDownloadCommand() *cobra.Command {
	opts := fanboxDownloadOptions{}
	cmd := &cobra.Command{
		Use:   "download SOURCE...",
		Short: "Download posts and their assets from FANBOX",
		Args:  requireMinArgs(1, "pixiv fanbox download SOURCE..."),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func (a app) fanboxDownload(cmd *cobra.Command, args []string, _ fanboxDownloadOptions) error {
	state, err := config.LoadSettingsState()
	if err != nil {
		return err
	}
...
func (a app) fanboxSavePostAssets(ctx context.Context, client *fanbox.Client, baseDir string, post fanbox.Post, seen map[string]struct{}) error {
	if post.Body == nil {
		return nil
	}
...
func (a app) newFanboxMCPCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "mcp",
		Short:   "Run the FANBOX MCP stdio server",
		Example: "pixiv fanbox mcp",
		Args:    requireExactArgs(0, "pixiv fanbox mcp"),
		RunE: func(cmd *cobra.Command, args []string) error {
			service, err := a.fanboxService()
			if err != nil {
				return err
