...
// TestReadEncryptedClassification 验证非空加密值的分类：darwin 上解密后无
// v10/v11 前缀 → ErrEncryptedFormatUnknown；其他平台 → ErrEncryptedValueUnsupported。
func TestReadEncryptedClassification(t *testing.T) {
	...
	if runtime.GOOS == "darwin" {
		if !errors.Is(err, core.ErrEncryptedFormatUnknown) {
			t.Fatalf("darwin err = %v, want ErrEncryptedFormatUnknown", err)
		}
	} else if !errors.Is(err, core.ErrEncryptedValueUnsupported) {
		t.Fatalf("err = %v, want ErrEncryptedValueUnsupported", err)
	}
}
