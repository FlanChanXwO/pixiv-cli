// Package core 定义浏览器 cookie provider 的共享类型与接口。
//
...
// New 按 browser 名称创建 provider：支持 "chrome"/"chromium"、"edge"、
// "firefox"、"safari"；未知名称返回 ErrUnknownBrowser。
// provider 子包通过 init 完成注册；推荐导入父包 browsercookies 获得完整分发。
...
// internal/storage/auth.SetReadAuthStoreFileForTest 一致。
