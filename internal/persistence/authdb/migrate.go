// Package authdb 管理 Pixiv / FANBOX 鉴权状态的单一 SQLite 数据库。
...
func init() {
	...
	if err != nil {
		panic("authdb: cannot read embedded migrations: " + err.Error())
	}
	for _, entry := range entries {
		...
		if err != nil {
			panic("authdb: cannot read embedded migration " + name + ": " + err.Error())
		}
...
func migrationVersion(filename string) int {
	...
	if underscore <= 0 {
		panic("authdb: malformed migration filename " + filename)
	}
	version, err := strconv.Atoi(filename[:underscore])
	if err != nil || version <= 0 {
		panic("authdb: malformed migration version in " + filename)
	}
	return version
}
