...
func TestRustUgoiraEncoderNativeGIFAndAPNG(t *testing.T) {
	...
	frames := []Frame{{File: "000000.jpg", Delay: 80}}

	gifPath := filepath.Join(dir, "out.gif")
	if err := NewRustEncoder().Encode(context.Background(), Input{
		ZipPath: zipPath, Frames: frames, WorkDir: dir, OutputPath: gifPath, Format: FormatGIF,
	}); err != nil {
	...
	if err := NewRustEncoder().Encode(context.Background(), Input{
		ZipPath: zipPath, Frames: frames, WorkDir: dir, OutputPath: apngPath, Format: FormatAPNG,
	}); err != nil {
...
func TestLinuxRustStaticlibLinkersLinkSystemMath(t *testing.T) {
	...
	for _, source := range []string{
		"internal/ugoira/rust_link_linux_amd64.go",
		"internal/ugoira/rust_link_linux_arm64.go",
	} {
...
func TestWindowsRustStaticlibSelectorsUseCgoLibrarySearchFlags(t *testing.T) {
	repoRoot := filepath.Clean(filepath.Join("..", ".."))
	for _, source := range []string{
		"internal/ugoira/rust_link_windows_amd64.go",
		"internal/ugoira/rust_link_windows_arm64.go",
	} {
		body, err := os.ReadFile(filepath.Join(repoRoot, source))
		if err != nil {
		...
		for _, want := range []string{
			"#cgo LDFLAGS: -L${SRCDIR}/../downloader/ugoira_rs/staticlib/",
			" -lugoira_rs",
			" -ladvapi32",
			" -lntdll",
			" -luserenv",
...
func TestPinnedRustSourcesDisableGitTextConversion(t *testing.T) {
	repoRoot := filepath.Clean(filepath.Join("..", ".."))
	for _, source := range []string{
		"internal/downloader/ugoira_rs/Cargo.toml",
		"internal/downloader/ugoira_rs/Cargo.lock",
		"internal/downloader/ugoira_rs/build.rs",
		"internal/downloader/ugoira_rs/.cargo/config.toml",
		"internal/downloader/ugoira_rs/src/lib.rs",
		"internal/downloader/ugoira_rs/src/nested/digest-input.rs",
		"internal/downloader/ugoira_rs/vendor/crc32fast/benches/bench.rs",
		"internal/downloader/ugoira_rs/vendor/crc32fast/src/specialized/mod.rs",
	...
	nonDigestSource := "internal/downloader/ugoira_rs/src/not-a-digest.md"
