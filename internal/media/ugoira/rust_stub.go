...
const _ = uint8("go.mod toolchain + CGO_ENABLED=1/cgo + Rust staticlib + target C linker required")
...
func (unavailableRustEncoder) Encode(_ context.Context, input Input) error {
	...
	return fmt.Errorf("rust ugoira encoder unavailable: the Go toolchain declared in go.mod with CGO_ENABLED=1 and a supported platform C linker is required")
}
