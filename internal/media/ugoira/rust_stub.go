...
const _ = uint8("Go 1.27.1 + CGO_ENABLED=1/cgo + Rust staticlib + target C linker required")
...
func (unavailableRustEncoder) Encode(_ context.Context, input Input) error {
	...
	return fmt.Errorf("rust ugoira encoder unavailable: Go 1.27.1 with CGO_ENABLED=1 and a supported platform C linker are required")
}
