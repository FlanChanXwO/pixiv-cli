...
func (s *Service) ImportAccount(ctx context.Context, refreshToken string, setDefault bool) (AccountSummary, error) {
	...
	return AccountSummary{UserID: credentials.UserID, Username: credentials.Username, Default: isDefault, Schedulable: true, Eligible: true, PoolStatusKnown: true}, nil
}
...
func (s *Service) CheckAccount(ctx context.Context, userID int64) (AccountSummary, error) {
	...
	return s.CheckRefreshToken(ctx, string(account.RefreshTokenCopy()))
}
...
func (s *Service) RestoreAccount(ctx context.Context, account AccountSummary, refreshToken string, setDefault bool) error {
	refreshToken = strings.TrimSpace(refreshToken)
