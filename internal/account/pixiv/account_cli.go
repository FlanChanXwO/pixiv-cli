...
// AccountStore 是 AccountService 依赖的 database-backed 账号能力，由
// Service 实现；接口让测试可以注入内存替身而不触发 OAuth 网络调用。
type AccountStore interface {
	ImportAccount(context.Context, string, bool) (AccountSummary, error)
	...
	CheckAccount(context.Context, int64) (AccountSummary, error)
	CheckRefreshToken(context.Context, string) (AccountSummary, error)
	ExportRefreshToken(context.Context, int64) (string, error)
	RefreshAccount(context.Context, int64) (AccountSummary, error)
	CurrentUser(context.Context) (*AccountSummary, error)
	RestoreAccount(context.Context, AccountSummary, string, bool) error
	AccountsWithTokens(context.Context) ([]AccountWithToken, error)
}

// AccountPoolStore 是账号池管理所需的可选能力；拆开接口以保持认证测试替身
// 不必实现 scheduler 写操作。
...
// AccountService 只适配 database-backed 的本地账号 API。账号文件、token rotation 和
// OAuth 身份验证均由 Service 维护，应用层不再复制另一份 auth store 调用链。
...
// AccountPoolStatusResult 是 auth pool status 的非 secret 输出。
type AccountPoolStatusResult struct {
	Enabled             bool                       `json:"enabled"`
	Strategy            config.AccountPoolStrategy `json:"strategy"`
...
func (s AccountService) ImportBundle(body []byte) (AccountBundleImportResult, error) {
	...
	for _, account := range bundle.Accounts {
		restoredAccount := AccountSummary{UserID: account.UserID, Username: account.Username}
...
// ListWithContext 返回本地账号及其非 secret pool 摘要。
...
// PoolStatus 返回运行时开关、策略以及本地账号调度摘要。
func (s AccountService) PoolStatus(ctx context.Context) (AccountPoolStatusResult, error) {
	...
	result := AccountPoolStatusResult{
		Enabled:  runtime.AccountPool.Enabled,
		Strategy: runtime.AccountPool.Strategy,
...
// SetPool 修改一组或全部本地账号的 schedulable 状态。
func (s AccountService) SetPool(ctx context.Context, userIDs []int64, schedulable, all bool) error {
	service, err := s.pixiv()
	if err != nil {
		return err
	}
	poolStore, ok := service.(AccountPoolStore)
	if !ok {
		return errors.New("pixiv account service does not support account pool management")
	}
	if all {
		if len(userIDs) != 0 {
			return errors.New("--all cannot be combined with UIDs")
		}
		return poolStore.SetAllPoolSchedulable(ctx, schedulable)
	}
	if len(userIDs) == 0 {
		return errors.New("at least one UID is required unless --all is used")
	}
	return poolStore.SetPoolSchedulable(ctx, userIDs, schedulable)
}
	...
	if !request.All {
		...
		for _, account := range accounts {
			if account.UserID == request.UserID {
				selected = []AccountWithToken{account}
				break
			}
		}
		if selected == nil {
...
// 轮换的 refresh token；再强制读取个人 profile，将 Premium 资格与检查时间写回 database。
...
func accountResultFromPixiv(account AccountSummary) AccountResult {
	...
	if account.PoolStatusKnown {
		...
		if account.PoolFrozenUntil != nil {
			frozenUntil := time.Unix(*account.PoolFrozenUntil, 0).UTC()
			result.PoolFrozenUntil = &frozenUntil
		}
	}
	return result
}

func accountImportResult(account AccountSummary, status string) AccountImportResult {
