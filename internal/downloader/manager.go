...
import (
	...
	"github.com/FlanChanXwO/pixiv-cli/internal/downloader/parallel"
...
type PixivClient = downloadapp.DownloadClient

// 下载结果属于应用层稳定 DTO；alias 让现有 MCP/下载包调用方保持源码兼容。
type DownloadedArtwork = downloadapp.DownloadedArtwork
type DownloadedFile = downloadapp.DownloadedFile
...
func (m *Manager) Download(ctx context.Context, request downloadapp.DownloadRequest) ([]DownloadedArtwork, error) {
...
func (m *Manager) downloadArtwork(ctx context.Context, id int64, pages []int, quality downloadapp.DownloadQuality, ugoiraFormat downloadapp.UgoiraFormat) (out DownloadedArtwork, err error) {
	...
	if kind == string(pixiv.ArtworkKindUgoira) {
		...
		if quality != downloadapp.DownloadQualityOriginal {
...
func (m *Manager) downloadUgoira(ctx context.Context, artwork pixiv.Artwork, base string, format downloadapp.UgoiraFormat) (string, error) {
