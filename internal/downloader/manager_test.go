...
import (
	...
	"github.com/FlanChanXwO/pixiv-cli/internal/downloader/filename"
	"github.com/FlanChanXwO/pixiv-cli/internal/downloader/ids"
...
func TestDownloadSingleArtworkReturnsPath(t *testing.T) {
	...
	got, err := m.Download(context.Background(), downloadapp.DownloadRequest{IllustIDs: []int64{42}})
	if err != nil {
...
func TestDownloadUsesSDKResourceReferenceAndDestination(t *testing.T) {
	...
	if _, err := m.Download(context.Background(), downloadapp.DownloadRequest{IllustIDs: []int64{42}}); err != nil {
		t.Fatalf("Download returned error: %v", err)
	}
...
func TestDownloadFailureDoesNotReplaceExistingFile(t *testing.T) {
	...
	_, err := m.Download(context.Background(), downloadapp.DownloadRequest{IllustIDs: []int64{42}})
	if err == nil {
...
func TestDownloadMultiPageArtworkReturnsAllPaths(t *testing.T) {
	...
	got, err := m.Download(context.Background(), downloadapp.DownloadRequest{IllustIDs: []int64{7}})
	if err != nil {
		t.Fatalf("Download returned error: %v", err)
	}
...
func TestDownloadUgoiraZipFailureCleansTemporaryZip(t *testing.T) {
	...
	_, err := m.Download(context.Background(), downloadapp.DownloadRequest{IllustIDs: []int64{9}})
	if err == nil {
		t.Fatal("Download returned nil error")
	}
...
func TestDownloadUgoiraReturnsFinalGIFOnly(t *testing.T) {
	...
	got, err := m.Download(context.Background(), downloadapp.DownloadRequest{IllustIDs: []int64{9}})
	if err != nil {
		t.Fatalf("Download returned error: %v", err)
	}
	if len(got) != 1 || len(got[0].Files) != 1 {
...
func TestDownloadUgoiraUsesOriginalArchive(t *testing.T) {
	...
	if _, err := m.Download(context.Background(), downloadapp.DownloadRequest{IllustIDs: []int64{9}}); err != nil {
...
func TestDownloadMissingPageMetadataReturnsError(t *testing.T) {
	...
	_, err := m.Download(context.Background(), downloadapp.DownloadRequest{IllustIDs: []int64{1}})
...
func TestDownloadUgoiraExplicitAPNGUsesRustEncoder(t *testing.T) {
	...
	got, err := m.Download(context.Background(), downloadapp.DownloadRequest{
		IllustIDs:    []int64{1},
		UgoiraFormat: downloadapp.UgoiraFormatAPNG,
	})
	if err != nil {
		t.Fatalf("Download returned error: %v", err)
	}
