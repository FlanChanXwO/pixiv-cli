...
func TestServerListsExpectedTools(t *testing.T) {
	...
	want := []string{
		...
		"user_detail", "user_artworks", "user_novels", "user_bookmarks", "user_novel_bookmarks", "user_following", "user_followers", "related_users", "blocked_users", "bookmark_tags", "bookmark_detail", "novel_bookmark_tags", "novel_bookmark_detail", "add_bookmark",
...
func TestOpenClientPassesCanceledContextAndClosesLease(t *testing.T) {
	...
	if calls.Load() != 2 {
		t.Fatalf("SDK factory calls=%d, want 2", calls.Load())
	}
	if err := lease.Close(); err != nil {
		t.Fatalf("close lease: %v", err)
	}
	if closeCalls.Load() != 1 {
		t.Fatalf("lease close calls=%d, want 1", closeCalls.Load())
	}
}
