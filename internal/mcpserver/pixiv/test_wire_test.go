...
// 本文件把 MCP Pixiv 测试的 typed fake 转为真实 public SDK client + wire
// responder：Open 端口返回经 pixiv.OpenWith 构造的 client，其 HTTP transport
// 按 App API path 分发并回放 canned wire JSON。请求参数会回填到 fake 的
// capture 字段，使既有断言保持 typed 语义。

// testSDKTransport 是 fakeSDKClient 驱动的 App API wire responder。
...
func (tr *testSDKTransport) RoundTrip(request *http.Request) (*http.Response, error) {
	...
	case "/v1/user/bookmarks/illust":
		...
		if tr.fake.userBookmarksErr != nil {
			return wireErrorResponse(tr.fake.userBookmarksErr)
		}
		status, body, err = tr.wireArtworkPage(sdk.Page[pixivsdk.Artwork]{Items: tr.fake.bookmarks})
	case "/v1/user/bookmarks/novel":
		req := pixivsdk.UserNovelBookmarksRequest{UserID: queryInt64(request.URL.Query(), "user_id"), Restrict: pixivsdk.Restrict(request.URL.Query().Get("restrict")), Tag: request.URL.Query().Get("tag"), Cursor: cursorFromOffset(queryInt(request.URL.Query(), "offset"))}
		tr.fake.userNovelBookmarksRequest = req
		tr.fake.novelBookmarksRequest = req
		page, callErr := callNovelPageFunc(tr.fake.userNovelBookmarks, req)
		if callErr != nil {
			return wireErrorResponse(callErr)
		}
		if tr.fake.userNovelBookmarks == nil && len(tr.fake.novelBookmarksPage.Items) > 0 {
			page = tr.fake.novelBookmarksPage
		}
		status, body, err = tr.wireNovelPage(page)
	...
	case "/v1/user/follower":
		...
		tr.fake.userFollowersRequest = req
		tr.fake.followersRequest = req
		page, callErr := callUserPageFunc(tr.fake.userFollowers, req)
		if callErr != nil {
			return wireErrorResponse(callErr)
		}
		if tr.fake.userFollowers == nil && len(tr.fake.followersPage.Items) > 0 {
			page = tr.fake.followersPage
		}
		status, body, err = tr.wireUserPreviewPage(page)
	...
	case "/v1/user/list":
		...
		tr.fake.blockedRequest = req
		page, callErr := callUserPageFunc(tr.fake.blockedUsers, req)
		if callErr != nil {
			return wireErrorResponse(callErr)
		}
		if tr.fake.blockedUsers == nil && len(tr.fake.blockedPage.Items) > 0 {
			page = tr.fake.blockedPage
		}
		status, body, err = tr.wireUserPreviewPage(page)
	...
	case "/v1/illust/bookmark/detail":
		...
		if tr.fake.artworkBookmarkErr != nil {
			return wireErrorResponse(tr.fake.artworkBookmarkErr)
		}
	...
	case "/v1/novel/series":
		req := pixivsdk.NovelSeriesRequest{SeriesID: queryInt64(request.URL.Query(), "series_id")}
		tr.fake.novelSeriesRequest = req
		page, callErr := callNovelPageFunc(tr.fake.novelSeries, req)
		if callErr != nil {
			return wireErrorResponse(callErr)
		}
		if tr.fake.novelSeries == nil && len(tr.fake.novelSeriesResult.Novels.Items) > 0 {
			page = tr.fake.novelSeriesResult.Novels
		}
		status, body, err = wireNovelSeries(tr.fake.novelSeriesResult.Series, page)
	...
	case "/v1/ugoira/metadata":
		req := pixivsdk.UgoiraMetadataRequest{ArtworkID: queryInt64(request.URL.Query(), "illust_id")}
		tr.fake.ugoiraMetadataRequest = req
		status, body, err = wireUgoiraMetadata(tr.fake.ugoiraMetadata)
...
func newTestSDKPorts(t *testing.T, fake *fakeSDKClient) (pixivmcpserver.SDKPorts, pixivmcpserver.Account) {
	...
	return pixivmcpserver.SDKPorts{
		...
		Pooled: func(ctx context.Context, _ pixivmcpserver.Account, attempt func(context.Context, *pixivsdk.Client) (bool, error)) error {
...
// --- wire envelope builders (upstream App API shape) ---
...
func wireCommentPage(comments []pixivsdk.Comment) (int, []byte, error) {
	items := make([]map[string]any, 0, len(comments))
	for _, item := range comments {
		items = append(items, wireCommentValue(item))
	}
	body, err := json.Marshal(struct {
...
func wireCommentValue(comment pixivsdk.Comment) map[string]any {
	...
	return map[string]any{
		...
		"user":       wireUser{ID: comment.User.ID, Name: comment.User.Name, Account: comment.User.Account},
...
func wireUgoiraMetadata(metadata pixivsdk.UgoiraMetadata) (int, []byte, error) {
	frames := make([]map[string]any, 0, len(metadata.Frames))
	for _, frame := range metadata.Frames {
		frames = append(frames, map[string]any{"file": frame.Filename, "delay": frame.DelayMilliseconds})
	}
	body, err := json.Marshal(struct {
		UgoiraMetadata map[string]any `json:"ugoira_metadata"`
	}{map[string]any{"frames": frames, "zip_url": ugoiraZipURL(metadata)}})
	return http.StatusOK, body, err
}

func wireNovelSeries(series pixivsdk.NovelSeries, page sdk.Page[pixivsdk.Novel]) (int, []byte, error) {
	dtos := make([]map[string]any, 0, len(page.Items))
	for _, item := range page.Items {
		dtos = append(dtos, wireNovelValue(item))
	}
	body, err := json.Marshal(struct {
		...
		"user":         wireUser{ID: series.User.ID, Name: series.User.Name, Account: series.User.Account},
		"is_concluded": series.IsConcluded,
	}, dtos, nil})
	return http.StatusOK, body, err
}

func toOptionalNovelSeriesDTO(series pixivsdk.NovelSeries) *pixivsdk.NovelSeriesDTO {
	converted := pixivsdk.ToNovelSeriesDTO(series)
	return &converted
}

// wireCommentPageResult 编码 comment page（含 total/access_control）。
...
func toOptionalBookmarkDetailDTO(detail pixivsdk.ArtworkBookmarkDetail) *pixivsdk.ArtworkBookmarkDetailDTO {
	converted := pixivsdk.ToArtworkBookmarkDetailDTO(detail)
	return &converted
}

func toOptionalUgoiraMetadataDTO(metadata pixivsdk.UgoiraMetadata) *pixivsdk.UgoiraMetadataDTO {
	converted := pixivsdk.ToUgoiraMetadataDTO(metadata)
	return &converted
}

func ugoiraZipURL(metadata pixivsdk.UgoiraMetadata) string {
	if len(metadata.Archives) > 0 {
		for _, archive := range metadata.Archives {
			if archive.Resource.URL != "" {
				return archive.Resource.URL
			}
		}
	}
	return ""
...
// wireArtworkSeriesPage 编码 artwork series 响应：series_detail + illusts。
