...
func handleSearchIllust(ctx context.Context, app *runtime.App, in searchIllustIn) (*mcp.CallToolResult, outputs.Records, error) {
	...
	if in.StartDate == "" && in.EndDate == "" {
		if startDate, endDate, ok := searchpixiv.QuickDateRange(in.Duration, time.Now()); ok {
			in.Duration = ""
	...
	if in.BookmarkMin != nil || in.BookmarkMax != nil || in.BookmarkStrategy != "" {
		...
		outcome, err := searchpixiv.SearchArtworks(ctx, app.PooledOperation(), searchpixiv.ArtworkSearchRequest{
			Query:      query,
			...
			Strategy:   searchpixiv.BookmarkFilterStrategy(in.BookmarkStrategy),
		})
		if err != nil {
			return outputs.Error(err)
		}
		...
		out := outputs.Records{
			...
			Filter:     outputs.BookmarkFilterFrom(outcome.Filter),
		}
