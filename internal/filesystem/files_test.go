...
func TestUserDataFileBuildsPathDirectlyUnderUserHome(t *testing.T) {
	...
	path, err := UserDataFile(AppDataDirName, "auth.json")
	require.NoError(t, err)

	assert.Equal(t, filepath.Join(home, AppDataDirName, "auth.json"), path)
}

func TestAppDataDirUsesConfiguredUserHome(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	...
	assert.Equal(t, filepath.Join(home, AppDataDirName), path)
}
...
func TestEnsurePrivateDirCreatesPrivateDirectory(t *testing.T) {
	...
	require.NoError(t, err)
	assertPlatformDirMode(t, info.Mode().Perm())
}
...
func TestWritePrivateFileCreatesParentAndFileWithPlatformPermissions(t *testing.T) {
	...
	require.NoError(t, WritePrivateFile(path, []byte("body"), PrivateFileMode))
...
func TestWritePrivateFileAppliesPlatformModeToExistingFile(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(filepath.Dir(path), PrivateDirMode))
	require.NoError(t, os.WriteFile(path, []byte("old"), 0o644))

	require.NoError(t, WritePrivateFile(path, []byte("new"), PrivateFileMode))
...
func assertPlatformFileMode(t *testing.T, actual os.FileMode) {
	...
	assert.Equal(t, os.FileMode(PrivateFileMode), actual)
}
...
func assertPlatformDirMode(t *testing.T, actual os.FileMode) {
	...
	assert.Equal(t, os.FileMode(PrivateDirMode), actual)
}
...
func TestReplaceUsesCanonicalReplacementAPI(t *testing.T) {
	dir := t.TempDir()
	source := filepath.Join(dir, "source.tmp")
	target := filepath.Join(dir, "target.txt")
	require.NoError(t, os.WriteFile(source, []byte("new"), 0o644))
