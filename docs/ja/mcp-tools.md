...
すべての entity read tool（search、detail、recommendation、user list、feed、MyPixiv）は structured `{records, pagination?}` を返します。各 Record は stable string の `id`、`type`、`url` を持ち、残りの public SDK field も保持します。record/schema/version field は追加しません。MCP text Content は短い summary だけで、entity JSON を複製しません。分類可能な entity-read error は `isError=true` と `records=[]` を返します。以下の entity 行はすべてこの contract を使用し、旧来の `items`/`illusts`/`user_previews` 形状は v0.8.0 の output ではありません。

Entity tool は `search_illust`、`search_novel`、`illust_detail`、`illust_related`、`illust_ranking`、`illust_recommended`、`recommended`、`illust_follow`、`novel_follow`、`illust_new`、`novel_new`、`mypixiv_users`、`mypixiv_illusts`、`mypixiv_novels`、`search_user`、`user_detail`、`user_artworks`、`user_novels`、`user_bookmarks`、`user_following` です。
...
| `download` | `src` または `srcs` のどちらか。各 source は PID、Pixiv artwork/user URL、または許可 CDN URL。任意の `pages`/`quality`/`concurrency`/`ugoira_format`（`gif` または `apng`）。`delivery` は `local_path` のみ | local file の path/file_uri/mime_type/page/size。image bytes は埋め込みません。 |
...
`download.src` は single source、`download.srcs` は input order を保つ source list で、両方の指定は invalid です。PID、official Pixiv artwork/user URL、または SDK の `ResourcePolicy` が許可する CDN URL を受け取ります。user URL は `illust`、`manga`、`ugoira` を展開し、novel は除外し App OAuth が必要です。`concurrency=0` は `2 × GOMAXPROCS`、正数はそのまま使います。HTTP cache metadata は `.pixiv-cache` に保存され、validator 一致の partial だけ `If-Range` で再開します。
...
| Tool | Input | Structured output |
| --- | --- | --- |
| `search_illust` | `word`、`search_target`、`sort`、`duration`、`page`、`limit`、`rating`、`content_type`、`ai_mode`、`aspect_ratio`、`resolution`、`tool` | `{records, pagination?}`。 |
| `search_novel` | `word`、`search_target`、`sort`、`duration`、`page`、`limit`、`rating`、`min_text_length`、`max_text_length`、`original_only` | App 専用の `{records, pagination?}`。分類可能な失敗は `isError=true`。 |
| `search_illust_options` | 必須 `word` | 検索語の `{tools,text}`。認証済み App 専用。 |
| `illust_detail` | `illust_id` | `{records}`。Pixiv が提供する場合は raw HTML `caption` を含みます。 |
| `illust_related` | `illust_id`、`page`、`limit` | `{records, pagination?}`。 |
| `illust_ranking` | `mode`、`date`、`page`、`limit` | `{records, pagination?}`。 |
| `illust_recommended` | `page`、`limit` | public SDK path による `{records, pagination?}`。 |
| `recommended` | 必須 `kind`（`all`、`illust`、`manga`、`novel`、`user`）、任意の `page`、`limit` | `{records, pagination?}`。`all` は 4 つの認証済み stream を順に読みます。 |
| `trending_tags_illust` | なし | trending tags。 |
| `illust_follow` | `restrict`、`page`、`limit` | `{records, pagination?}`。認証が必要です。 |
| `search_user` | `word`、`page`、`limit` | `{records, pagination?}`。認証済み App では `app_search`、匿名 fallback では `related_illust_authors`。後者は username search ではありません。 |
| `user_detail` | 必須 `user_id` | `{records}`。認証済み App 専用です。 |
| `user_artworks` | 任意の `user_id`、`type`、`page`、`limit` | `{records, pagination?}`。UID の省略時は認証済み user を使います。 |
| `user_bookmarks` | 任意の `user_id`、`restrict`、`tag`、`page`、`limit` | `{records, pagination?}`。 |
| `user_following` | 任意の `user_id`、`restrict`、`page`、`limit` | `{records, pagination?}`。 |
