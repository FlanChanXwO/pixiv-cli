...
`.github/workflows/ci.yml` 只承载不受信的 `Quality gate`：它执行 PR 代码时只有仓库只读权限，也不再负责 dispatch worker。`.github/workflows/pr-metadata.yml` 是受信的 `pull_request_target` coordinator，只从 base branch 运行策略代码：验证 PR 模板与 verification declaration、分类 PR diff，发布 `PR template gate` / `PR commands gate` commit status，以及 `Platform smoke gate` / `Container smoke gate` Check Run。需要 smoke 时它只 dispatch PR base 分支上的 worker，不执行 PR 代码。`.github/ci-change-scope.gitignore` 是唯一的路径策略源：普通 pattern 表示纯文档，`?pattern` 表示只需 Quality，`!pattern` 表示需要 Quality + Platform + Container。不需要的 Quality 与 smoke gate 都以真实 `Skipped` 呈现，不再伪装成 success。
