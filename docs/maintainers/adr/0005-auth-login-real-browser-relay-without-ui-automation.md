...
- If the URL handler path is unavailable, use the normal browser flow with its loopback/manual callback page.
- Authorization runs in the normal system browser, and the callback bridge submits the current login callback to the CLI listener.
...
- For a headless SSH server, keep the listener on server loopback and let an explicit local `ssh -L` tunnel carry browser-page submissions back to it.

## Consequences

- The CLI can reuse the existing browser login state on supported desktop systems.
