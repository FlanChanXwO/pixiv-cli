...
> the protected `release` Environment, and isolated credentials are configured. v0.4.0 has been published as an
...
libraries and manifest are complete — for example, the current official release can be installed with `@v0.4.0`;
...
`checksums.json`. v0.4.0 ships the complete asset set; later versions should only be trusted as direct download
...
protected `release` Environment and a controlled macOS Keychain recovery copy. v0.4.0 is the current signed
