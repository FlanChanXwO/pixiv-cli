...
`.github/workflows/ci.yml` owns only the untrusted `Quality gate`: it runs PR code with read-only repository access and never dispatches workers. `.github/workflows/pr-metadata.yml` is the trusted `pull_request_target` coordinator. From base-branch code it validates the PR template and verification declaration, classifies the PR diff, publishes `PR template gate` / `PR commands gate` commit statuses plus `Platform smoke gate` / `Container smoke gate` Check Runs, and dispatches required smoke workers on the PR base branch without executing PR code. `.github/ci-change-scope.gitignore` is the only path-policy source: plain patterns are documentation-only, `?pattern` requires Quality only, and `!pattern` requires Quality + Platform + Container. Unneeded Quality and smoke gates are real skipped checks rather than synthetic successes.
