...
func newError(operation string, code sdk.Code, detail string) *sdk.Error {
...
func classifyOAuthError(err error, operation string) *sdk.Error {
	...
	if !errors.As(err, &failure) {
		...
		return sdk.NewError(product, operation, sdk.CodeCredentialsExpired, sdk.WithCause(redactedCause(err)))
	}
	switch failure.Kind {
	case protocol.FailureHTTPStatus:
		switch failure.StatusCode {
		case http.StatusBadRequest:
			return sdk.NewError(product, operation, sdk.CodeCredentialsExpired, sdk.WithHTTPStatus(failure.StatusCode), sdk.WithRetry(retryAdvice(failure)))
		case http.StatusUnauthorized:
			return sdk.NewError(product, operation, sdk.CodeCredentialsExpired, sdk.WithHTTPStatus(failure.StatusCode), sdk.WithRetry(retryAdvice(failure)))
		case http.StatusForbidden:
			return sdk.NewError(product, operation, sdk.CodeForbidden, sdk.WithHTTPStatus(failure.StatusCode))
		case http.StatusTooManyRequests:
			return sdk.NewError(product, operation, sdk.CodeRateLimited, sdk.WithHTTPStatus(failure.StatusCode), sdk.WithRetry(retryAdvice(failure)))
		case http.StatusInternalServerError, http.StatusBadGateway, http.StatusServiceUnavailable, http.StatusGatewayTimeout:
			return sdk.NewError(product, operation, sdk.CodeUpstreamError, sdk.WithHTTPStatus(failure.StatusCode))
		default:
			return sdk.NewError(product, operation, sdk.CodeUpstreamError, sdk.WithHTTPStatus(failure.StatusCode))
		}
	case protocol.FailureMalformed:
		return sdk.NewError(product, operation, sdk.CodeMalformedUpstreamResponse)
	case protocol.FailureRejected:
		return sdk.NewError(product, operation, sdk.CodeUpstreamError)
	case protocol.FailureForbidden:
		return sdk.NewError(product, operation, sdk.CodeForbidden)
	default:
...
// Content operations without a valid token map to CodeCredentialsExpired;
...
func classifyAppError(err error, operation string) *sdk.Error {
	...
	if !errors.As(err, &failure) {
		...
		return sdk.NewError(product, operation, sdk.CodeUpstreamError, sdk.WithCause(redactedCause(err)))
	}
	switch failure.Kind {
	case protocol.FailureHTTPStatus:
...
func classifyStatus(failure protocol.Failure, operation string) *sdk.Error {
	switch failure.StatusCode {
	case http.StatusBadRequest:
		return sdk.NewError(product, operation, sdk.CodeInvalidArgument, sdk.WithHTTPStatus(failure.StatusCode))
	case http.StatusUnauthorized:
		return sdk.NewError(product, operation, sdk.CodeCredentialsExpired, sdk.WithHTTPStatus(failure.StatusCode), sdk.WithRetry(retryAdvice(failure)))
	case http.StatusForbidden:
		return sdk.NewError(product, operation, sdk.CodeForbidden, sdk.WithHTTPStatus(failure.StatusCode))
	case http.StatusNotFound:
		return sdk.NewError(product, operation, sdk.CodeNotFound, sdk.WithHTTPStatus(failure.StatusCode))
	case http.StatusGone:
		return sdk.NewError(product, operation, sdk.CodeContentUnavailable, sdk.WithHTTPStatus(failure.StatusCode))
	case http.StatusTooManyRequests:
		return sdk.NewError(product, operation, sdk.CodeRateLimited, sdk.WithHTTPStatus(failure.StatusCode), sdk.WithRetry(retryAdvice(failure)))
	case http.StatusInternalServerError, http.StatusBadGateway, http.StatusServiceUnavailable, http.StatusGatewayTimeout:
		return sdk.NewError(product, operation, sdk.CodeUpstreamError, sdk.WithHTTPStatus(failure.StatusCode))
...
func classifyTransport(failure protocol.Failure, operation string) *sdk.Error {
	...
	return sdk.NewError(product, operation, sdk.CodeUpstreamUnavailable, opts...)
}
