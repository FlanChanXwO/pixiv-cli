...
func TestAccountPromptFlows(t *testing.T) {
	...
	stdout.Reset()
	stderr.Reset()
	code = Run([]string{"pixiv", "account", "use"}, strings.NewReader(""), &stdout, &stderr)
	require.Equal(t, 0, code, stderr.String())

	stdout.Reset()
	stderr.Reset()
	code = Run([]string{"pixiv", "account", "remove"}, strings.NewReader(""), &stdout, &stderr)
	require.Equal(t, 0, code, stderr.String())
...
func TestAccountRemovePromptCancelKeepsData(t *testing.T) {
	authPath, _ := useTempPaths(t)
	require.NoError(t, saveAuthStore(authPath, authStore{
		DefaultAccount: "main",
		Accounts:       []account{{Name: "main", RefreshToken: "main-token"}},
	}))
	setPromptStub(t, promptStub{
		selects:  []string{"main"},
		confirms: []bool{false},
...
func TestAccountLoginBrowserFailureFallsBackToTerminalPrompt(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	code := Run([]string{"pixiv", "account", "login", "--addr", addr, "--timeout", "5s"}, strings.NewReader(""), &stdout, &stderr)
...
func setTestCLIClientFactory(t *testing.T, factory func(clientConfig) (*pixiv.Client, error)) {
	t.Helper()
	old := newCLIClient
