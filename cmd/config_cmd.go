...
func (a app) configGet(alias string) error {
	...
	if err != nil {
		return fmt.Errorf("%w. valid keys: %s", err, strings.Join(validSettingAliases(), ", "))
	}
	if !value.HasValue {
...
func (a app) configSet(alias, raw string) error {
	...
	if !ok {
		return fmt.Errorf("unknown config key %q. valid keys: %s", alias, strings.Join(validSettingAliases(), ", "))
	}
	...
	if err := setConfigValue(path, alias, value); err != nil {
		return err
	}
	if envRaw, ok := envValue(spec); ok {
...
func (a app) configUnset(alias string) error {
	...
	_, err = unsetConfigValue(path, alias)
	if err != nil {
		return err
	}
	if envRaw, ok := envValue(spec); ok {
