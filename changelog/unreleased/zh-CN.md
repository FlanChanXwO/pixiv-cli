...
- 新增认证 App API 小说搜索：CLI `pixiv novel search WORD`、public SDK `SearchNovel` 与 MCP `search_novel` 同步支持关键词匹配、日期排序、时间范围、分级、正文长度和仅原创筛选；小说结果提供稳定作品 URL、分级、正文长度与原创标记。
...
- OAuth operation diagnostics 现在保留安全的 `transport_kind` 分类（包括 typed network timeout），使没有 HTTP 状态码的上游失败可被诊断，同时不记录 URL、凭据、header 或响应 body。
