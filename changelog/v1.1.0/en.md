...
- Consolidate manual release approval into one final publication gate. Build, SDK E2E, provenance, source-trust, production, and container checks run automatically; one `release-approval` Environment approval then unlocks GitHub Release publication and the downstream registry/Homebrew publication path without additional approval prompts. ([#85](https://github.com/FlanChanXwO/pixiv-cli/pull/85))
