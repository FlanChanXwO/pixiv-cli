...
2. Run `pixiv auth list --json` to see configured local accounts. Presence in
   ...
   an error: the documented anonymous web fallback remains available when
   `web_fallback_enabled=true`.

## Hard rules

1. NEVER directly read, print, echo, or persist a raw refresh/access token
   outside the CLI-managed account store. `auth list` / `auth check` outputs
   are safe (they never contain tokens), while supported `auth add` / `auth
   login` flows may save credentials through the CLI.
2. Run interactive `pixiv auth login` only when the user explicitly asks and
   is present to complete browser OAuth.
3. NEVER pass a token as a command-line argument (`--token`, `--refresh-token`
   with a literal value) — it leaks into shell history. If the user wants to
   import an existing token, ask them to run `pixiv auth add` themselves and
   paste the token at the interactive prompt.
4. NEVER accept or forward cookies (`PHPSESSID`, `refresh_token=...` cookie
...
1. **Reduce at the source (preferred):** always pass `--limit N` on list
   commands; add `--page`, `--tag`, `--type`, `--rating`, or other search
   filters as needed.
...
pixiv user bookmarks|following [USER_ID] --limit 20
pixiv bookmark add|remove ILLUST_ID       # write op
...
Global flags on data commands: `--uid UID` (pick local account),
...
3. **`--limit` semantics:** unset = one upstream batch (compat default);
   positive = keep fetching until N matches; `0` = fetch everything.
   `--page` requires a positive `--limit`. `--offset` is deprecated.
