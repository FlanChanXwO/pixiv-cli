...
Use `vector sync local PATH`, `vector sync bookmarks`, `vector search QUERY_OR_IMAGE`, `vector status`, and `vector rebuild` only for an explicit indexing or retrieval request. Confirm gallery/image scope before reading local paths; results can contain private paths. Sync/search require a preinstalled Python/SigLIP2 runtime; never install dependencies or download weights implicitly. Bookmark sync uses the current account and includes public/private bookmark covers without artwork detail requests. Ordinary artwork reads passively record already-fetched identities without extra Pixiv requests. Rebuild currently re-embeds recorded local images only; Pixiv rebuild is not yet supported. Search emits NDJSON, with the best page per Pixiv artwork, not canonical Records for detail/download pipelines.
