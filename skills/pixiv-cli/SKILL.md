...
Use `vector sync local PATH`, `vector sync bookmarks`, `vector search QUERY_OR_IMAGE`, `vector status`, and `vector rebuild` only for an explicit indexing or retrieval request. Confirm gallery/image scope before reading local paths; results can contain private paths. Sync/search require a preinstalled Python/SigLIP2 runtime; never install dependencies or download weights implicitly. Bookmark sync uses the current account, records public/private bookmark covers without artwork detail requests, and also embeds pages the passive observer recorded earlier (all pages, via persisted resource identities). Ordinary artwork reads passively record already-fetched identities without extra Pixiv requests; failures stay diagnosable on stderr without changing output. Rebuild re-embeds recorded local images and Pixiv pages together; Pixiv pages re-fetch through persisted stable identities (requires an authenticated account, local-only indexes never initialize one). Search emits NDJSON, with the best page per Pixiv artwork, not canonical Records for detail/download pipelines.
